...
function CupModuleScreen({ screen, onNav }) {
  const [clubs] = useState(CUP_DEFAULT_CLUBS);
  const [teams, setTeams] = useState(CUP_DEFAULT_TEAMS);
  ...
  useEffect(() => {
    let mounted = true;
    Promise.all([
      cupLoadShared("teams", CUP_DEFAULT_TEAMS),
      cupLoadShared("matches", []),
      cupLoadShared("orders", {}),
      cupLoadShared("announcements", []),
      cupLoadShared("sponsors", []),
      cupLoadShared("lunchWindows", []),
    ]).then(([nextTeams, nextMatches, nextOrders, nextAnnouncements, nextSponsors, nextLunch]) => {
      if (!mounted) return;
      setTeams(Array.isArray(nextTeams) && nextTeams.length ? nextTeams : CUP_DEFAULT_TEAMS);
      setMatches(Array.isArray(nextMatches) ? nextMatches : []);
      setOrders(nextOrders || {});
      setAnnouncements(Array.isArray(nextAnnouncements) ? nextAnnouncements : []);
      setSponsors(Array.isArray(nextSponsors) ? nextSponsors : []);
      setLunchWindows(Array.isArray(nextLunch) ? nextLunch : []);
      setLoading(false);
    });
  ...
  const pitches = [...new Set(matches.map((m) => m.pitch).filter(Boolean))];
  const groups = cupComputeGroups(teams, matches);
  const teamById = (id) => teams.find((t) => t.id === id) || { name: id || "TBC" };
  const foodOrders = Object.values(orders || {}).filter(Boolean);
  const burgerTotal = foodOrders.reduce((sum, o) => sum + Number(o.players || 0) + Number(o.mentors || 0), 0);
  const sausageTotal = foodOrders.reduce((sum, o) => sum + Number(o.sausageRolls || 0), 0);

  if (loading) {
    return (
      <div style={{ flex: 1, overflow: "auto", background: P.soft }}>
        <CupTopBar title="Cup" sub="Loading tournament data…" />
        <div style={{ padding: 24, fontFamily: F.body, color: P.muted }}>Loading…</div>
      </div>
    );
  }
  ...
  if (screen === "cup-events") {
    return (
      <div style={{ flex: 1, overflow: "auto", background: P.soft }}>
        <CupTopBar title="Events" sub="Create and manage blitzes and tournaments">
          <CupBtn label="+ Create Event" />
        </CupTopBar>
        <div style={{ padding: 24 }}>
          <CupCard style={{ padding: 20 }}>
            <div style={{ display: "flex", justifyContent: "space-between", gap: 16, alignItems: "flex-start" }}>
              <div>
                <div style={{ fontFamily: F.body, fontSize: 10, fontWeight: 900, color: CUP_ORANGE, textTransform: "uppercase", letterSpacing: ".08em" }}>Current event</div>
                <div style={{ fontFamily: F.display, fontSize: 22, fontWeight: 900, color: P.ink, marginTop: 5 }}>{CUP_EVENT.name}</div>
                <div style={{ fontFamily: F.body, fontSize: 11, color: P.muted, marginTop: 6 }}>{CUP_EVENT.venue}</div>
                <div style={{ fontFamily: F.body, fontSize: 11, color: P.muted, marginTop: 3 }}>{CUP_EVENT.date}</div>
              </div>
              <span style={{ background: "#ecfdf3", color: "#16803c", borderRadius: 999, padding: "6px 10px", fontFamily: F.body, fontSize: 9, fontWeight: 900 }}>ACTIVE</span>
            </div>

            <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 10, marginTop: 18 }}>
              {[["Clubs", clubs.length], ["Teams", teams.length], ["Fixtures", matches.length], ["Finals", finals.length]].map(([label, value]) => (
                <div key={label} style={{ background: P.soft, borderRadius: 10, padding: 12 }}>
                  <div style={{ fontFamily: F.body, fontSize: 9, color: P.muted }}>{label}</div>
                  <strong style={{ fontFamily: F.display, fontSize: 20, color: P.ink }}>{value}</strong>
                </div>
              ))}
            </div>

            <div style={{ display: "flex", gap: 8, marginTop: 16 }}>
              <CupBtn label="Manage Teams" variant="ghost" onClick={() => onNav("cup-teams")} />
              <CupBtn label="Open Schedule" variant="ghost" onClick={() => onNav("cup-schedule")} />
            </div>
          </CupCard>
        </div>
      </div>
  ...
  if (screen === "cup-teams") {
    return (
      <div style={{ flex: 1, overflow: "auto", background: P.soft }}>
        <CupTopBar title="Teams" sub={`${clubs.length} clubs · ${teams.length} competition teams`}>
          <CupBtn label="+ Add Guest Team" />
        </CupTopBar>
        <div style={{ padding: 24 }}>
          <div className="cup-team-grid" style={{ display: "grid", gridTemplateColumns: "repeat(3,minmax(0,1fr))", gap: 14 }}>
            {clubs.map((club) => {
              const clubTeams = teams.filter((t) => t.clubId === club.id);
              const lunch = lunchWindows.find((w) => (w.clubs || []).includes(club.id));
              return (
                <CupCard key={club.id} style={{ padding: 16 }}>
                  <div style={{ fontFamily: F.display, fontSize: 15, fontWeight: 900, color: P.ink }}>{club.name}</div>
                  <div style={{ fontFamily: F.body, fontSize: 10, color: P.muted, marginTop: 3 }}>{club.town} · {club.county}</div>
                  <div style={{ marginTop: 12 }}>
                    {clubTeams.map((team) => (
                      <div key={team.id} style={{ display: "flex", justifyContent: "space-between", padding: "9px 0", borderTop: `1px solid ${P.line}`, fontFamily: F.body, fontSize: 10 }}>
                        <strong style={{ color: P.ink }}>{team.name}</strong>
                        <span style={{ color: P.muted }}>{matches.filter((m) => m.teamA === team.id || m.teamB === team.id).length} fixtures</span>
                      </div>
                    ))}
                  </div>
                  <div style={{ marginTop: 8, borderRadius: 9, padding: 9, background: P.soft, fontFamily: F.body, fontSize: 9, color: P.muted }}>
                    Lunch: <strong style={{ color: P.ink }}>{lunch ? `${lunch.from}–${lunch.to}` : "TBC"}</strong>
                  </div>
                </CupCard>
              );
            })}
  ...
  if (screen === "cup-schedule") {
    return (
      <div style={{ flex: 1, overflow: "auto", background: P.soft }}>
        <CupTopBar title="Schedule" sub={`${matches.length} fixtures · ${pitches.length || 3} pitches`}>
          <CupBtn label="Generate Schedule" />
        </CupTopBar>
        <div style={{ padding: 24 }}>
          <CupCard style={{ overflow: "hidden" }}>
            <div style={{ display: "grid", gridTemplateColumns: "80px 100px 1fr 120px", padding: "10px 14px", background: P.soft, fontFamily: F.body, fontSize: 9, fontWeight: 900, color: P.muted, textTransform: "uppercase" }}>
              <span>Time</span><span>Pitch</span><span>Fixture</span><span>Stage</span>
            </div>
            {matches.length === 0 ? (
              <div style={{ padding: 20, fontFamily: F.body, fontSize: 11, color: P.muted }}>No fixtures generated yet.</div>
            ) : matches.map((m, index) => (
              <div key={m.id || index} style={{ display: "grid", gridTemplateColumns: "80px 100px 1fr 120px", padding: "11px 14px", borderTop: `1px solid ${P.line}`, alignItems: "center", fontFamily: F.body, fontSize: 10 }}>
                <strong style={{ color: P.ink }}>{m.time || "TBC"}</strong>
                <span style={{ color: P.muted }}>{m.pitch || "—"}</span>
                <span style={{ color: P.ink, fontWeight: 700 }}>{m.finalLabel === "Presentations" ? "Presentations" : `${teamById(m.teamA).name} v ${teamById(m.teamB).name}`}</span>
                <span style={{ color: m.finalLabel ? CUP_ORANGE : P.muted, fontWeight: 800 }}>{m.finalLabel || "Group"}</span>
              </div>
            ))}
          </CupCard>
        </div>
  ...
  if (screen === "cup-results") {
    return (
      <div style={{ flex: 1, overflow: "auto", background: P.soft }}>
        <CupTopBar title="Results" sub="Live standings and completed fixtures" />
        <div style={{ padding: 24 }}>
          <div className="cup-results-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
            {groups.length === 0 ? (
              <CupCard style={{ padding: 18 }}><div style={{ fontFamily: F.body, color: P.muted, fontSize: 11 }}>Tables will appear once fixtures are generated.</div></CupCard>
            ) : groups.map((group, groupIndex) => {
              const rows = cupComputeStandings(group, matches);
              const grade = group[0]?.id?.endsWith("A") ? "A" : "B";
              return (
                <CupCard key={groupIndex} style={{ overflow: "hidden" }}>
                  <div style={{ padding: "13px 15px", background: grade === "A" ? CUP_ORANGE_SOFT : "#f5f3ff", fontFamily: F.display, fontWeight: 900, color: P.ink }}>
                    Group {groupIndex + 1} · {grade} teams
                  </div>
                  {rows.map((row, index) => (
                    <div key={row.id} style={{ display: "grid", gridTemplateColumns: "28px 1fr 30px 34px", gap: 8, padding: "9px 14px", borderTop: `1px solid ${P.line}`, fontFamily: F.body, fontSize: 10 }}>
                      <strong style={{ color: index < 2 ? CUP_ORANGE : P.muted }}>{index + 1}</strong>
                      <span style={{ color: P.ink, fontWeight: 700 }}>{teamById(row.id).name}</span>
                      <span style={{ color: P.muted }}>{row.played}</span>
                      <strong style={{ color: P.ink }}>{row.points}</strong>
                    </div>
                  ))}
                </CupCard>
              );
  ...
  if (screen === "cup-participant-view") {
    return (
      <div style={{ flex: 1, overflow: "auto", background: P.soft }}>
        <CupTopBar title="Participant View" sub="Preview the mobile event-day experience">
          <CupBtn label="Open Preview" />
        </CupTopBar>
        <div style={{ padding: 24 }}>
          <div style={{ display: "grid", gridTemplateColumns: "1.2fr .8fr", gap: 16 }}>
            <CupCard style={{ padding: 18 }}>
              <div style={{ fontFamily: F.display, fontSize: 17, fontWeight: 900, color: P.ink }}>What participants see</div>
              {["Event-day home", "Club selection", "Fixtures", "Live standings", "Team schedule", "Food orders", "Event info & sponsors", "Announcements"].map((item) => (
                <div key={item} style={{ padding: "9px 0", borderTop: `1px solid ${P.line}`, fontFamily: F.body, fontSize: 10, color: P.ink }}>✓ {item}</div>
              ))}
            </CupCard>
            <CupCard style={{ padding: 18 }}>
              <div style={{ fontFamily: F.display, fontSize: 17, fontWeight: 900, color: P.ink }}>Publish status</div>
              <div style={{ marginTop: 10, fontFamily: F.body, fontSize: 10, color: P.muted, lineHeight: 1.6 }}>Participant content uses the same live Cup data as the organiser screens.</div>
            </CupCard>
          </div>
        </div>
      </div>
  ...
  if (screen === "cup-settings") {
    return (
      <div style={{ flex: 1, overflow: "auto", background: P.soft }}>
        <CupTopBar title="Settings" sub="Tournament setup and event-day content" />
        <div style={{ padding: 24 }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
            <CupCard style={{ padding: 18 }}>
              <div style={{ fontFamily: F.display, fontSize: 17, fontWeight: 900, color: P.ink }}>Event timing</div>
              {[["Registration", CUP_EVENT.registration], ["First throw-in", CUP_EVENT.firstThrowIn], ["Target finish", CUP_EVENT.targetFinish]].map(([label, value]) => (
                <div key={label} style={{ display: "flex", justifyContent: "space-between", padding: "10px 0", borderTop: `1px solid ${P.line}`, fontFamily: F.body, fontSize: 10 }}>
                  <span style={{ color: P.muted }}>{label}</span><strong style={{ color: P.ink }}>{value}</strong>
                </div>
              ))}
            </CupCard>
            <CupCard style={{ padding: 18 }}>
              <div style={{ fontFamily: F.display, fontSize: 17, fontWeight: 900, color: P.ink }}>Event content</div>
              {[["Announcements", announcements.length], ["Sponsors", sponsors.length], ["Lunch sittings", lunchWindows.length], ["Burger vouchers", burgerTotal], ["Breakfast orders", sausageTotal]].map(([label, value]) => (
                <div key={label} style={{ display: "flex", justifyContent: "space-between", padding: "10px 0", borderTop: `1px solid ${P.line}`, fontFamily: F.body, fontSize: 10 }}>
                  <span style={{ color: P.muted }}>{label}</span><strong style={{ color: P.ink }}>{value}</strong>
                </div>
              ))}
  ...
  // Dashboard
  const upcoming = matches.filter((m) => m.status !== "finished" && m.finalLabel !== "Presentations").slice(0, 6);
  return (
    <div style={{ flex: 1, overflow: "auto", background: P.soft }}>
      <CupTopBar title="Dashboard" sub={`${CUP_EVENT.name} · ${CUP_EVENT.date}`}>
        <CupBtn label="+ New Event" onClick={() => onNav("cup-events")} />
      </CupTopBar>

      <style>{`
        @media (max-width: 1050px) {
          .cup-dashboard-stats { grid-template-columns: repeat(2,1fr) !important; }
          .cup-dashboard-main { grid-template-columns: 1fr !important; }
          .cup-team-grid { grid-template-columns: repeat(2,1fr) !important; }
        }
        @media (max-width: 620px) {
          .cup-dashboard-stats, .cup-team-grid, .cup-results-grid { grid-template-columns: 1fr !important; }
        }
      `}</style>

      <div style={{ padding: "20px 24px" }}>
        <div className="cup-dashboard-stats" style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 14, marginBottom: 20 }}>
          <CupStatCard label="Clubs" value={clubs.length} sub={`${teams.length} competition teams`} color={CUP_ORANGE} icon="🏑" />
          <CupStatCard label="Fixtures" value={matches.length} sub={`${finished.length} finished · ${live.length} live`} color="#fb8c00" icon="📅" />
          <CupStatCard label="Pitches" value={pitches.length || 3} sub={pitches.length ? pitches.join(" · ") : "3 event pitches"} color="#29b6f6" icon="📍" />
          <CupStatCard label="Event Status" value={matches.length ? "Ready" : "Setup"} sub={`${announcements.length} announcements · ${sponsors.length} sponsors`} color="#43a047" icon="✓" />
        </div>

        <div className="cup-dashboard-main" style={{ display: "grid", gridTemplateColumns: "1.3fr .8fr", gap: 20 }}>
          <CupCard style={{ padding: 18 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
              <div>
                <div style={{ fontFamily: F.display, fontSize: 17, fontWeight: 900, color: P.ink }}>Upcoming fixtures</div>
                <div style={{ fontFamily: F.body, fontSize: 10, color: P.muted, marginTop: 3 }}>Live Cup schedule</div>
              </div>
              <button onClick={() => onNav("cup-schedule")} style={{ border: 0, background: "transparent", color: CUP_ORANGE, fontWeight: 900, cursor: "pointer" }}>View schedule</button>
            </div>
            {upcoming.length === 0 ? (
              <div style={{ padding: 16, background: P.soft, borderRadius: 10, fontFamily: F.body, color: P.muted, fontSize: 11 }}>No fixtures generated yet.</div>
            ) : upcoming.map((m, index) => (
              <div key={m.id || index} style={{ display: "grid", gridTemplateColumns: "70px 90px 1fr", gap: 10, padding: "10px 0", borderTop: index ? `1px solid ${P.line}` : "none", fontFamily: F.body, fontSize: 10 }}>
                <strong style={{ color: P.ink }}>{m.time || "TBC"}</strong>
                <span style={{ color: P.muted }}>{m.pitch || "TBC"}</span>
                <span style={{ color: P.ink, fontWeight: 700 }}>{m.finalLabel === "Presentations" ? "Presentations" : `${teamById(m.teamA).name} v ${teamById(m.teamB).name}`}</span>
              </div>
            ))}
          </CupCard>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            <CupCard style={{ padding: 18 }}>
              <div style={{ fontFamily: F.display, fontSize: 17, fontWeight: 900, color: P.ink }}>Event overview</div>
              {[["Venue", CUP_EVENT.venue], ["Registration", CUP_EVENT.registration], ["First throw-in", CUP_EVENT.firstThrowIn], ["Target finish", CUP_EVENT.targetFinish]].map(([label, value]) => (
                <div key={label} style={{ padding: "10px 0", borderTop: `1px solid ${P.line}` }}>
                  <div style={{ fontFamily: F.body, fontSize: 9, color: P.muted, textTransform: "uppercase", fontWeight: 800 }}>{label}</div>
                  <div style={{ fontFamily: F.body, fontSize: 11, color: P.ink, fontWeight: 700, marginTop: 4 }}>{value}</div>
                </div>
              ))}
            </CupCard>

            <CupCard style={{ padding: 18 }}>
              <div style={{ fontFamily: F.display, fontSize: 17, fontWeight: 900, color: P.ink }}>Event-day snapshot</div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginTop: 12 }}>
                <div style={{ background: P.soft, borderRadius: 10, padding: 12 }}>
                  <div style={{ fontFamily: F.body, fontSize: 9, color: P.muted }}>BURGERS</div>
                  <strong style={{ fontFamily: F.display, fontSize: 22, color: P.ink }}>{burgerTotal}</strong>
                </div>
                <div style={{ background: P.soft, borderRadius: 10, padding: 12 }}>
                  <div style={{ fontFamily: F.body, fontSize: 9, color: P.muted }}>BREAKFAST</div>
                  <strong style={{ fontFamily: F.display, fontSize: 22, color: P.ink }}>{sausageTotal}</strong>
                </div>
              </div>
            </CupCard>
          </div>
...
export default function App() {
  ...
  return (
    <div style={{ display: "flex", minHeight: "100vh", width: "100%", fontFamily: F.body, paddingTop: showMobile ? 62 : 0, paddingBottom: showMobile ? 68 : 0, boxSizing: "border-box" }}>
      ...
      {/* CUP screens */}
      {screen.startsWith("cup-") && <CupModuleScreen screen={screen} onNav={setScreen} />}
