...
export default function ParentUpdates({userId,players=[],selectedPlayer}){
  ...
  async function loadEvents(){
    if(!teamIds.length)return setEvents([]);
    const {data}=await supabase.from("club_events").select("*,facility:facilities(name)").in("age_group_id",teamIds).gte("starts_at",new Date(Date.now()-86400000).toISOString()).order("starts_at").limit(40); setEvents(data||[]);
    if(userId){const {data:r}=await supabase.from("availability_responses").select("*").eq("parent_user_id",userId);setResponses(r||[]);}
  }
  useEffect(()=>{loadEvents();},[userId,teamIds.join(",")]);
  async function respond(event,response){
    const child=(players||[]).find(p=>p.age_group_id===event.age_group_id) || selectedPlayer;
