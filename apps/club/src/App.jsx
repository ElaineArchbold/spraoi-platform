...
function modulesForRole(role, capabilities = {}) {
  ...
  if (normalized === "lead_coach") {
    return ["coach", "club", "academy", "connect"];
  }
...
function ClubCoachesScreen({ club, ageGroups, coaches, selectedTeam, onReloadCoaches, userRole, currentUserId }) {
  ...
  const [personPermissionCoach, setPersonPermissionCoach] = useState(null);
  const [personPermissionAssignments, setPersonPermissionAssignments] = useState([]);
  const [personPermissionOverrides, setPersonPermissionOverrides] = useState({});
  const [personPermissionLoading, setPersonPermissionLoading] = useState(false);
  const [personPermissionSaving, setPersonPermissionSaving] = useState(false);

  function permissionRoleKey(role = "") {
    const value = String(role || "").toLowerCase();

    if (value === "lead_coach") return "lead_coach";
    if (value === "team_admin") return "team_admin";
    if (value === "coach") return "coach";
    if (value === "mentor") return "mentor";
    if (value === "coach_mentor") return "coach";

    return value;
  }

  function rolePresetPermissions(role) {
    const preset = SYSTEM_TEAM_ROLE_PRESETS.find(
      (item) => item.key === permissionRoleKey(role)
    );

    return new Set(preset?.permissions || []);
  }

  function currentPersonPermissionState(row, permission) {
    const override =
      personPermissionOverrides?.[row.id]?.[permission];

    if (override === "allow") return "allow";
    if (override === "deny") return "deny";

    return rolePresetPermissions(row.role).has(permission)
      ? "role"
      : "none";
  }

  function changePersonPermission(
    staffId,
    permission,
    value
  ) {
    setPersonPermissionOverrides((current) => {
      const next = {
        ...(current || {}),
        [staffId]: {
          ...((current || {})[staffId] || {})
      ...
      if (value === "allow" || value === "deny") {
        next[staffId][permission] = value;
      } else {
        delete next[staffId][permission];
      }
  ...
  async function openPersonPermissions(coach) {
    if (!coach?.id || !club?.id) return;

    setPersonPermissionCoach(coach);
    setPersonPermissionLoading(true);
    setPersonPermissionOverrides({});

    try {
      const rows = assignmentsForCoach(coach.id).map(
        (row) => ({
          ...row,
          team:
            row.team ||
            (ageGroups || []).find(
              (team) =>
                String(team.id) ===
                String(row.age_group_id)
            ) ||
            null
        })
      );

      setPersonPermissionAssignments(rows);
      ...
      const { data, error } = await supabase
        ...
        .select(
          "id,team_staff_id,permission_key,effect"
        )
        .in(
          "team_staff_id",
          rows.map((row) => row.id)
      ...
      (data || []).forEach((override) => {
        if (!next[override.team_staff_id]) {
          next[override.team_staff_id] = {};
        }
      ...
      setPersonPermissionOverrides(next);
    } catch (error) {
      console.error(
        "Could not load person permissions:",
        error
      );
      ...
      setPersonPermissionLoading(false);
    }
  }
  ...
  async function savePersonPermissions() {
    if (
      ...
      personPermissionSaving
    ) {
      return;
    }

    setPersonPermissionSaving(true);
    setMessage("");

    try {
      for (const assignment of personPermissionAssignments) {
        const overrides =
          personPermissionOverrides?.[assignment.id] || {};

        const { error: deleteError } = await supabase
          .from("team_permission_overrides")
          .delete()
          .eq("team_staff_id", assignment.id);
        ...
        const inserts = Object.entries(overrides)
          ...
          .map(([permission_key, effect]) => ({
            club_id: club.id,
            age_group_id: assignment.age_group_id,
            team_staff_id: assignment.id,
            permission_key,
            effect
          }));
      ...
      setPersonPermissionCoach(null);
      setPersonPermissionAssignments([]);
      setPersonPermissionOverrides({});
    } catch (error) {
      console.error(
        "Could not save person permissions:",
        error
      );
      ...
      setPersonPermissionSaving(false);
    }
  }
                ...
                {isAdmin && (
                  <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                    <Btn
                      label="Permissions"
                      variant="ghost"
                      onClick={() => openPersonPermissions(coach)}
                    />
                    <Btn
                      label="Remove"
                      variant="ghost"
      ...
      {personPermissionCoach && (
        ...
        >
          ...
          >
            ...
            >
              Role permissions are the defaults. Use overrides
              only when this person needs an exception.
            </div>

            {personPermissionLoading ? (
              <div
                style={{
                  padding: 28,
                  textAlign: "center",
              ...
              >
                {personPermissionAssignments.map((row) => {
                  const inherited =
                    rolePresetPermissions(row.role);

                  const role =
                    SYSTEM_TEAM_ROLE_PRESETS.find(
                      (item) =>
                        item.key ===
                        permissionRoleKey(row.role)
                    );
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        {TEAM_PERMISSION_OPTIONS.map(
                          ([permission, label]) => {
                            const fromRole =
                              inherited.has(permission);

                            const state =
                              currentPersonPermissionState(
                                row,
                                permission
                              );
            ...
            >
              ...
              <Btn
                ...
                disabled={
                  personPermissionSaving ||
                  personPermissionLoading ||
                  personPermissionAssignments.length === 0
                }
                onClick={savePersonPermissions}
              />
            </div>
          </div>
        </div>
