...
function dateForWeekday(weekStart, weekday) {
  // weekStart is the Monday of the displayed week.
  // weekday uses 1 = Monday ... 7 = Sunday.
  const base = new Date(`${weekStart}T12:00:00`);

  const monday = new Date(base);
  const currentDay = monday.getDay(); // Sun=0, Mon=1 ...
  const daysSinceMonday = (currentDay + 6) % 7;

  monday.setDate(monday.getDate() - daysSinceMonday);

  const result = new Date(monday);
  result.setDate(monday.getDate() + (Number(weekday) - 1));
...
function localDateTime(date, time) {
  ...
  const local = new Date(value.getFullYear(), value.getMonth(), value.getDate(), hour, minute || 0, 0, 0);
...
export default function ClubScheduling({ club, ageGroups = [], currentUserId, hideHeader = false }) {
  ...
  async function loadAll(){
    ...
    try {
      const [f,s,a] = await Promise.all([
        ...
        supabase.from("recurring_training_slots").select("*,facility:facilities(id,name,location)").eq("club_id",club.id).eq("active",true).order("weekday").order("start_time"),
  ...
  async function addSlot(){
    ...
    const payload={club_id:club.id,age_group_id:teamId,facility_id:facilityId,weekday:Number(weekday),start_time:startTime,end_time:endTime,effective_from:isoDate(new Date()),active:true};
    const query = editingSlotId
      ? supabase.from("recurring_training_slots").update(payload).eq("id", editingSlotId)
      : supabase.from("recurring_training_slots").insert(payload);
    const {error}=await query;
    if(error){ setMessage(error.message); return; }
    setMessage(editingSlotId ? "Recurring training slot updated." : "Recurring training slot added.");
  ...
  function beginEditSlot(slot) {
    ...
    setTeamId(slot.age_group_id || "");
    setFacilityId(slot.facility_id || "");
    setWeekday(String(slot.weekday || 1));
    setStartTime(String(slot.start_time || "18:00").slice(0, 5));
    setEndTime(String(slot.end_time || "19:00").slice(0, 5));
    setTab("recurring");
  }

  async function removeRecurringSlot(slot) {
    if (!slot?.id) return;
    if (!window.confirm("Remove this recurring slot? Existing weekly allocations and published sessions will remain unchanged.")) return;
    const { error } = await supabase.from("recurring_training_slots").update({ active: false, updated_at: new Date().toISOString() }).eq("id", slot.id);
    if (error) { setMessage(error.message); return; }
    if (editingSlotId === slot.id) setEditingSlotId(null);
    setMessage("Recurring slot removed. Existing week records were left unchanged.");
    await loadAll();
  }
  ...
  async function publishWeek(){
    ...
    for(const a of allocations.filter(x=>x.status!=="cancelled")){
      ...
      try {
        linkedSession = await ensureSessionForAllocation({ ...a, status: "published" });
      } catch (syncError) {
        console.error("Could not sync published pitch allocation into Coach session", syncError);
        setMessage(`Pitch plan could not be fully published: ${syncError?.message || "session sync failed"}`);
        setBusy(false);
        return;
      }
      const {data:existingEvent}=await supabase.from("club_events").select("*").eq("training_allocation_id",a.id).maybeSingle();
      const eventPayload={club_id:club.id,age_group_id:a.age_group_id,event_type:"training",title:"Training",facility_id:a.facility_id,location:a.facility?.name||null,starts_at:a.starts_at,ends_at:a.ends_at,status:existingEvent?"changed":"scheduled",source:"club_allocation",training_allocation_id:a.id,session_id: linkedSession?.id || null,created_by:currentUserId};
      ...
      if(linkedSession?.id){
        const confirmedLocation = a.facility?.name || facilities.find((facility) => String(facility.id) === String(a.facility_id))?.name || null;
        await supabase.from("sessions").update({
          weekly_allocation_id: a.id,
          confirmed_starts_at: a.starts_at,
          confirmed_ends_at: a.ends_at,
          confirmed_facility_id: a.facility_id || null
    ...
    // Email each Lead Coach only the allocations for the team(s) they lead.
    const { data: clubLeadStaff } = await supabase
      .from("team_staff")
      .select("user_id,age_group_id,coach:coaches(email,name)")
      .eq("club_id", club.id)
      .eq("role", "lead_coach")
      .eq("status", "active");

    const allocationsByTeam = new Map();
    for (const allocation of publishedAllocations) {
      const key = String(allocation.age_group_id || "");
      if (!key) continue;
      const rows = allocationsByTeam.get(key) || [];
      rows.push(allocation);
      allocationsByTeam.set(key, rows);
    }

    const emailFailures = [];
    let emailsSent = 0;

    for (const staffRow of clubLeadStaff || []) {
      const recipient = String(staffRow.coach?.email || "").trim().toLowerCase();
      const staffTeamId = String(staffRow.age_group_id || "");
      const teamAllocations = allocationsByTeam.get(staffTeamId) || [];
      if (!recipient || !teamAllocations.length) continue;

      const summaryLines = [...teamAllocations]
        .sort(
          (a, b) =>
        ...
        .map((allocation) => {
          ...
          const facility =
            ...
            facilities.find(
              (item) => String(item.id) === String(allocation.facility_id)
            )?.name ||
            "Venue TBC";

          const when = new Date(allocation.starts_at).toLocaleString("en-IE", {
            weekday: "short",
            day: "numeric",
            month: "short",
            hour: "2-digit",
      ...
      const team = ageGroups.find(
        (item) => String(item.id) === staffTeamId
      );

      const { data: emailResult, error: emailError } =
        await supabase.functions.invoke("send-spraoi-notification-email", {
          body: {
            recipients: [recipient],
            subject: `Spraoi: ${teamName(team)} pitch allocation published`,
            title: `${teamName(team)} training allocation`,
            message: [
              `The pitch allocation for ${teamName(team)} for the week beginning ${new Date(
                `${weekStart}T12:00:00`
            ...
            actionUrl: `${window.location.origin}/?module=coach&screen=coach-planner&team=${staffTeamId}`,
          },
        });

      if (emailError || !emailResult?.ok) {
        emailFailures.push(
          `${recipient}: ${emailError?.message || emailResult?.error || "email failed"}`
        );
      } else {
        emailsSent += 1;
      }
    ...
    if (emailFailures.length) {
      setMessage(
        `Weekly allocations published. ${emailsSent} Lead Coach email${
          emailsSent === 1 ? "" : "s"
        } sent; ${emailFailures.length} failed.`
      );
    } else if (emailsSent) {
      setMessage(
        `Weekly allocations published. ${emailsSent} Lead Coach email${
          emailsSent === 1 ? "" : "s"
        } sent to the relevant team coach${emailsSent === 1 ? "" : "es"}.`
      );
    } else {
      setMessage("Weekly allocations published. No matching Lead Coach email recipients were found.");
    }
        ...
        <>
          ...
          <div
            style={{
              ...card,
              marginBottom: 14,
              borderColor: slots.length ? "#fecaca" : LINE,
            }}
          >
            <div
              style={{
            ...
            >
              <div>
                <div style={sectionTitle}>Recurring slots for this week</div>
                <div style={sectionSub}>
                  These are the club's normal weekly allocations. They remain templates until you generate the week's drafts, then they appear as editable pitch allocations for approval and publishing.
                </div>
              </div>
              <span
                style={{
                  fontSize: 10,
                  fontWeight: 800,
                  color: slots.length ? RED : MUTED,
                  background: slots.length ? "#fef2f2" : SOFT,
                  borderRadius: 999,
                  padding: "6px 9px",
                }}
              >
                {slots.length} recurring slot{slots.length === 1 ? "" : "s"}
              </span>
            </div>
            ...
            {slots.length ? (
              ...
              >
                {slots.map((slot) => {
                  const team = ageGroups.find(
                    (item) => String(item.id) === String(slot.age_group_id)
                  );
                  const facility = facilities.find(
                    (item) => String(item.id) === String(slot.facility_id)
                  );
                  const weekdayLabel = [
                    "",
                    "Monday",
                    ...
                    "Saturday",
                    "Sunday",
                  ][Number(slot.weekday)] || "Weekly";

                  const alreadyGenerated = allocations.some(
                    (allocation) =>
                      String(allocation.recurring_slot_id || "") ===
                      String(slot.id)
                  );
                  ...
                  return (
                    <div
                      ...
                      style={{
                        padding: "10px 11px",
                        border: `1px solid ${
                          alreadyGenerated ? "#bbf7d0" : LINE
                        }`,
                        borderRadius: 11,
                        background: alreadyGenerated ? "#f0fdf4" : SOFT,
                      }}
                    >
                      <div
                        style={{
                      ...
                      >
                        {weekdayLabel} · {String(slot.start_time || "").slice(0, 5)}–{String(slot.end_time || "").slice(0, 5)} · {facility?.name || "Pitch TBC"}
                      </div>
                      <div
                        style={{
                          fontSize: 8.5,
                          fontWeight: 800,
                          color: alreadyGenerated ? "#15803d" : RED,
                          marginTop: 6,
                        }}
                      >
                        {alreadyGenerated
                          ? "✓ Draft created for this week"
                          : "Recurring template — Generate Drafts to add it to the planner"}
                      </div>
                      <div style={{ display: "flex", gap: 6, marginTop: 8 }}>
                        <button type="button" onClick={() => beginEditSlot(slot)} style={{ ...btn(false), height: 30, padding: "0 9px", fontSize: 9 }}>Edit</button>
                        <button type="button" onClick={() => removeRecurringSlot(slot)} style={{ ...btn(false), height: 30, padding: "0 9px", fontSize: 9, color: RED }}>Remove</button>
                      </div>
                    </div>
                  );
                })}
              ...
              >
                No recurring training slots are configured. Add them under Recurring Slots first.
              </div>
            )}
          </div>
        ...
        <>
          ...
          >
            ...
            >
              ...
              {editingSlotId && (
                <button type="button" onClick={() => setEditingSlotId(null)} style={btn(false)}>Cancel</button>
              )}
            </div>
          </div>
                ...
                slotRows.map((slot) => (
                  ...
                  >
                    ...
                    <div style={{ display: "flex", gap: 6, alignItems: "center", flexWrap: "wrap" }}>
                      <span style={{ padding: "5px 8px", borderRadius: 999, background: SOFT, color: INK, fontSize: 9, fontWeight: 800 }}>
