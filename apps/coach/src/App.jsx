              ...
              teamSessions.slice(0, 4).map((sess, i) => (
                <div key={sess.id || i} onClick={() => onOpenSession(sess)} style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 0", borderTop: i > 0 ? `1px solid ${P.line}` : "none", cursor: "pointer" }}>
                  <div style={{ width: 40, textAlign: "center", background: P.soft, borderRadius: 8, padding: "6px 0" }}>
                    ...
                    <div style={{ fontFamily: F.display, fontSize: 16, fontWeight: 800, color: P.ink }}>{sess.session_date ? new Date(`${sess.session_date}T12:00:00`).getDate() : "?"}</div>
                  </div>
                  <div style={{ flex: 1 }}>
              ...
              {calDays.map((day, i) => {
                ...
                return (
                  <div key={day} onClick={() => clickDate(day)} className={`coach-planner-day${sessions?.length ? " has-session" : ""}`} style={{ minHeight: 58, minWidth: 0, padding: 4, borderRadius: 7, border: sessions?.length ? `1.5px solid ${P.p300}` : (todayBorder ? `2px solid ${P.p600}` : `1px solid ${P.line}`), background: sessions?.length ? P.p50 : (todayBorder ? P.p50 : P.white), cursor: "pointer", overflow: "hidden", position:"relative" }}>
                    ...
                    {sessions?.length > 0 && (
                      <div className="coach-planner-session-count" style={{ position:"absolute", top:4, right:4, minWidth:16, height:16, padding:"0 4px", borderRadius:8, background:P.p600, color:"#fff", fontFamily:F.body, fontSize:8, fontWeight:800, display:"grid", placeItems:"center", boxSizing:"border-box" }}>
                        {sessions.length}
                      </div>
                    )}
...
function SessionBuilderScreen({ club, ageGroups, skills, allActivities, coaches, diagramMap, selectedTeam, onNav, editingSession, onClearEdit }) {
  ...
  useEffect(() => {
    ...
    async function loadPublishedAllocation() {
      ...
      const confirmedTime = new Date(allocation.starts_at).toTimeString().slice(0, 5);
  ...
  async function saveSession(forceSave = false) {
    ...
    try {
      ...
      let connectEventId = editingSession?.event_id || null;
      let connectStartsAt = editingSession?.planned_starts_at || (plannedStartTime ? `${sessionDate}T${plannedStartTime}:00` : null);
      let connectEndsAt = null;
      let connectLocation = plannedLocation.trim() || editingSession?.planned_location || null;
        ...
        if (allocation) {
          ...
          if (event?.id) {
            ...
            await supabase.from("sessions").update({
              event_id: event.id,
              planned_starts_at: allocation.starts_at,
              planned_location: connectLocation,
            }).eq("id", sessionId);
          }
        } else {
          const plannedStart = plannedStartTime ? `${sessionDate}T${plannedStartTime}:00` : null;
          ...
          if (plannedStart) {
            ...
            if (event?.id) {
              connectEventId = event.id;
              await supabase.from("sessions").update({ event_id: event.id }).eq("id", sessionId);
            }
          }
        }
          ...
          <div style={{ background: P.white, borderRadius: 12, padding: 14, border: `1px solid ${P.line}`, marginBottom: 14 }}>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1.5fr", gap:8, marginTop:10 }}>
              <div>
                ...
                <input type="time" value={plannedStartTime} disabled={Boolean(publishedAllocation)} onChange={(e)=>setPlannedStartTime(e.target.value)} style={{ width:"100%", boxSizing:"border-box", height:36, borderRadius:8, border:`1px solid ${P.line}`, padding:"0 9px", fontFamily:F.body, background:publishedAllocation?P.soft:P.white }} />
              </div>
              <div>
                ...
                <input list="spraoi-club-facilities" value={plannedLocation} disabled={Boolean(publishedAllocation)} onChange={(e)=>setPlannedLocation(e.target.value)} placeholder="Choose a Club facility or enter another location" style={{ width:"100%", boxSizing:"border-box", height:36, borderRadius:8, border:`1px solid ${P.line}`, padding:"0 9px", fontFamily:F.body, background:publishedAllocation?P.soft:P.white }} />
        ...
        {sessions.map((sess) => (
          <div key={sess.id} onClick={() => onOpenSession(sess)} style={{ background: P.white, borderRadius: 12, padding: "14px 18px", border: `1px solid ${P.line}`, marginBottom: 8, display: "flex", alignItems: "center", gap: 14, cursor: "pointer", boxShadow: Sh.card }}>
            ...
            <div style={{ display: "flex", gap: 6 }}>
              ...
              <Btn label="Edit" icon={<CoachUiIcon name="edit" size={16} />} variant="secondary" style={{ height: 28, fontSize: 10 }} onClick={(e) => { e.stopPropagation(); onEditSession(sess); }} />
            </div>
          </div>
        ))}
      </div>
      ...
      <div style={{ padding: 24, maxWidth: 1180, margin: "0 auto", display: "grid", gap: 16 }}>
        <AcademyCard>
          ...
          <div style={{ display: "flex", gap: 8, marginTop: 14, flexWrap: "wrap" }}>
            ...
            <button onClick={addPlayer} disabled={saving || !name.trim() || !selectedTeam?.id || !club?.id} style={{ height: 38, border: 0, borderRadius: 10, background: ACADEMY_BLUE, color: "#fff", padding: "0 16px", fontFamily: F.body, fontSize: 11, fontWeight: 800, cursor: "pointer", opacity: saving || !name.trim() || !selectedTeam?.id || !club?.id ? .55 : 1 }}>{saving ? "Adding…" : "＋ Add player"}</button>
          </div>
