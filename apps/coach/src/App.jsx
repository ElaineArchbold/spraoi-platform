...
async function syncConnectSessionDraft({ club, selectedTeam, sessionId, eventId, sessionDate, startsAt, endsAt, location }) {
  if (!sessionId || !selectedTeam?.id || !club?.id) return;
  try {
    const { data: authData } = await supabase.auth.getUser();
    const userId = authData?.user?.id || null;
    if (!userId) return;
    const dateValue = sessionDate ? new Date(`${sessionDate}T12:00:00`) : (startsAt ? new Date(startsAt) : null);
    const dateLabel = dateValue && !Number.isNaN(dateValue.getTime())
      ? dateValue.toLocaleDateString("en-IE", { weekday: "long", day: "numeric", month: "long" })
      : "Date TBC";
    const timeFmt = (value) => {
      if (!value) return null;
      const d = new Date(value);
      return Number.isNaN(d.getTime()) ? null : d.toLocaleTimeString("en-IE", { hour: "2-digit", minute: "2-digit", hour12: false });
    };
    const startLabel = timeFmt(startsAt);
    const endLabel = timeFmt(endsAt);
    const timeLabel = startLabel ? `${startLabel}${endLabel ? ` - ${endLabel}` : ""}` : "Time TBC";
    const teamLabel = teamDisplayName(selectedTeam);
    const safeLocation = String(location || "").trim() || "Location TBC";
    const title = `Training — ${teamLabel} — ${dateLabel}`;
    const body = [
      `Training for ${teamLabel}`,
      `Date: ${dateLabel}`,
      `Time: ${timeLabel}`,
      `Location: ${safeLocation}`,
      "",
      "Please respond Yes / Maybe / No in Spraoi Connect so the coaching team can plan attendance.",
      "",
      "Audience: whole team by default — change this to a subgroup in Connect if this session is for a selected squad.",
      "Please bring the usual training gear and water.",
    ].join("\n");

    let existing = null;
    let existingQuery = supabase.from("connect_messages")
      .select("id")
      .eq("club_id", club.id)
      .eq("age_group_id", selectedTeam.id)
      .eq("message_type", "coach_session_draft")
      .is("sent_at", null);
    existingQuery = eventId ? existingQuery.eq("event_id", eventId) : existingQuery.eq("title", title);
    ...
    const payload = {
      ...
      audience_type: "team",
      message_type: "coach_session_draft",
      title,
      body,
      sent_at: null,
    };
    if (existing?.id) await supabase.from("connect_messages").update(payload).eq("id", existing.id);
    else await supabase.from("connect_messages").insert(payload);
  } catch (draftError) {
    console.warn("Connect session draft could not be created:", draftError?.message || draftError);
  }
}
...
function SpraoiNavIcon({ name = "", size = 18 }) {
  ...
  else if (key.includes("dashboard")) shape=<><path d="M3 11.5 12 4l9 7.5"/><path d="M5.5 10.5V20h13v-9.5"/><path d="M9.5 20v-6h5v6"/></>;
...
function coachIconAsset(name) {
  const map = {
    dashboard: "/icons/coach/dashboard.svg",
    planner: "/icons/coach/planner.svg",
    sessions: "/icons/coach/sessions.svg",
    drills: "/icons/coach/drills.svg",
    ...
    edit: "/icons/coach/edit.svg",
    delete: "/icons/coach/delete.svg",
    duplicate: "/icons/coach/duplicate.svg",
    video: "/icons/coach/video.svg",
    location: "/icons/coach/location.svg",
    time: "/icons/coach/time.svg",
  };
  return map[name] || "/icons/coach/dashboard.svg";
}

function CoachUiIcon({ name, size = 20, className = "" }) {
  return <img src={coachIconAsset(name)} alt="" aria-hidden="true" className={className} style={{ width: size, height: size, objectFit: "contain" }} />;
}

function SecondarySidebarIcon({ moduleKey, id }) {
  if (moduleKey === "coach" && id === "coach-attendance") {
    return (
      <span
        className="spraoi-secondary-nav-icon spraoi-attendance-inline-icon"
        style={{
          display: "grid",
          placeItems: "center",
          color: "#7C3AED"
...
function TopBar({ title, sub, children }) {
  ...
  const background = isAcademy
    ...
    : isConnect
      ...
      : key === "coach"
        ? "linear-gradient(135deg, #fcfaff 0%, #f5efff 52%, #eadcff 100%)"
...
function Btn({ label, variant = "primary", icon, onClick, style }) {
  const styles = {
    primary: { background: "#7C3AED", color: P.white, border: "1px solid #7C3AED", boxShadow: "0 5px 14px rgba(16,36,62,.12)" },
  ...
  return (
    <div className="spraoi-stat-card" style={{ background: P.white, borderRadius: 14, padding: "16px 18px", border: `1px solid ${P.line}`, borderTop: `3px solid ${color}`, boxShadow: Sh.card }}>
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 8 }}>
        ...
        {icon && (String(icon).startsWith("/") ? <span className="spraoi-card-icon-chip" style={{ "--card-accent": color }}><img className="spraoi-card-icon" src={icon} alt="" aria-hidden="true" /></span> : <span className="spraoi-card-icon-fallback" style={{ "--card-accent": color }}>{icon}</span>)}
      </div>
...
function DashboardScreen({ club, ageGroups, planSessions, weeklyPlan, upcomingSessions, onNav, onOpenSession, allActivities, selectedTeam, favouriteIds = [], coaches = [], canEdit = false, onPermissionDenied = () => {} }) {
  ...
  return (
    ...
    <div style={{ flex: 1, overflow: "auto", background: P.soft }}>
      ...
      <TopBar title="Dashboard" sub={today}>
        <Btn label="New Session" variant="primary" onClick={() => canEdit ? onNav("coach-builder") : onPermissionDenied()} />
      </TopBar>
      <div style={{ padding: "20px 24px" }}>
        ...
        >
          ...
          >
            ...
            >
              Coaches
            </div><span className="coach-card-icon spraoi-card-icon-chip" style={{"--card-accent":P.p600}}><img className="spraoi-card-icon" src="/icons/coach/coaches.svg" alt=""/></span>
          ...
          >
            ...
            >
              Favourite Drills
            </div><span className="coach-card-icon spraoi-card-icon-chip" style={{"--card-accent":P.p600}}><img className="spraoi-card-icon" src="/icons/coach/favourite.svg" alt=""/></span>
          ...
          >
            ...
            >
              ...
              </span><span className="coach-card-icon spraoi-card-icon-chip" style={{"--card-accent":P.sky}}><img className="spraoi-card-icon" src="/icons/coach/team.svg" alt=""/></span>
            </div>
          ...
          >
            ...
            <span className="coach-card-icon spraoi-card-icon-chip" style={{"--card-accent":"#2563EB"}}><img className="spraoi-card-icon" src="/icons/coach/academy.svg" alt=""/></span>
        ...
        <section className="spraoi-dashboard-analytics coach-dashboard-analytics">
          <div className="spraoi-dashboard-section-head">
            <div>
              <div className="spraoi-dashboard-section-title">Coaching overview</div>
              <div className="spraoi-dashboard-section-sub">Session focus, squad readiness and coaching library for the selected team.</div>
            </div>
            <button className="spraoi-download-button coach-download" type="button" onClick={()=>{
              const rows=[
                ["Metric","Value"],
                ["Coaches",(coaches||[]).length],
                ["Favourite drills",favouriteDrills.length],
                ["Players",panelPlayers.length],
                ["Football A",footballACount],
                ["Football B",footballBCount],
                ["Hurling/Camogie A",hurlingACount],
                ["Hurling/Camogie B",hurlingBCount],
                ["Skills focus %",40],
                ["Game Play focus %",30],
                ["Conditioning focus %",20],
              ];
              ...
              a.href=url;
              a.download="coach-dashboard.csv";
              a.click();
              URL.revokeObjectURL(url);
            }}>Download</button>
          </div>

          <div className="spraoi-dashboard-chart-grid coach-overview-grid">
            <div className="spraoi-chart-card">
              <div className="spraoi-chart-card-title">Squad grouping</div>
              <div className="spraoi-bar-chart">
                <div>
                  <div className="spraoi-bar-label"><span>Football grouped</span><strong>{footballACount+footballBCount}</strong></div>
                  <div className="spraoi-bar-track"><div className="spraoi-bar-fill" style={{width:`${panelPlayers.length?Math.min(100,((footballACount+footballBCount)/panelPlayers.length)*100):0}%`,background:P.p600}}/></div>
                </div>
                <div>
                  <div className="spraoi-bar-label"><span>{selectedTeam?.gender==="girls"?"Camogie":"Hurling"} grouped</span><strong>{hurlingACount+hurlingBCount}</strong></div>
                  <div className="spraoi-bar-track"><div className="spraoi-bar-fill" style={{width:`${panelPlayers.length?Math.min(100,((hurlingACount+hurlingBCount)/panelPlayers.length)*100):0}%`,background:"#2563EB"}}/></div>
                </div>
              </div>
            </div>

            <div className="spraoi-chart-card coach-overview-session-focus">
              <div className="spraoi-chart-card-title">Session Focus</div>
              <div className="spraoi-donut-wrap">
                <div
                  className="spraoi-donut coach-focus-donut"
                  style={{background:`conic-gradient(${P.p600} 0% 40%, ${P.orange} 40% 70%, ${P.sky} 70% 90%, ${P.line} 90% 100%)`}}
                >
                  <div className="spraoi-donut-centre">
                    <strong>40%</strong>
                    <span>skills</span>
                  </div>
                </div>
                <div className="spraoi-chart-legend">
                  <div><span className="spraoi-legend-dot" style={{background:P.p600}}/><span>Skills</span><strong>40%</strong></div>
                  <div><span className="spraoi-legend-dot" style={{background:P.orange}}/><span>Game Play</span><strong>30%</strong></div>
                  <div><span className="spraoi-legend-dot" style={{background:P.sky}}/><span>Conditioning</span><strong>20%</strong></div>
                  <div><span className="spraoi-legend-dot" style={{background:P.line}}/><span>Other</span><strong>10%</strong></div>
                </div>
              </div>
            </div>

            <div className="spraoi-chart-card">
              <div className="spraoi-chart-card-title">Coaching library</div>
              <div className="spraoi-mini-kpis">
                <div><img src="/icons/coach/coaches.svg" alt=""/><span>Coaches</span><strong>{(coaches||[]).length}</strong></div>
                <div><img src="/icons/coach/favourite.svg" alt=""/><span>Favourite drills</span><strong>{favouriteDrills.length}</strong></div>
                <div><img src="/icons/coach/team.svg" alt=""/><span>Players</span><strong>{panelPlayers.length}</strong></div>
              </div>
            </div>
          </div>
        </section>
        <div className="coach-dashboard-main-grid" style={{ display: "grid", gridTemplateColumns: "minmax(0, 1fr) minmax(0, 1fr)", gap: 20, marginBottom: 20 }}>          {/* Upcoming Sessions */}
          <div style={{ background: P.white, borderRadius: 14, padding: 18, border: `1px solid ${P.line}`, boxShadow: Sh.card }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
              <div style={{ fontFamily: F.display, fontSize: 15, fontWeight: 800, color: P.ink }}>
                Upcoming Sessions
              </div>

              <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
                <button
                  onClick={() => onNav("coach-planner")}
                  style={{
                    background: "none",
                    border: "none",
                    fontFamily: F.body,
            ...
            {(!teamSessions || teamSessions.length === 0) ? (
              ...
              teamSessions.slice(0, 4).map((sess, i) => {
                const sessionDate = sess.session_date
                  ? new Date(`${sess.session_date}T12:00:00`)
                  : null;
                  ...
                  >
                    ...
                    <div style={{ flex: 1 }}>
                      <div style={{
                        ...
                        {sess.plan?.hurling_skill?.name || "Training Session"}
                      </div>
                      ...
                      <div style={{
                        ...
                        {sessionDate
                          ? sessionDate.toLocaleDateString("en-IE", {
                              weekday: "long",
                              day: "numeric",
                              month: "short"
                            })
                          : ""}
                        {" - "}
                        {sess.total_duration_mins || "?"}min
                      </div>

                      {(sess.confirmed_facility?.name ||
                        sess.facility?.name ||
                        sess.location) && (
                        <div style={{
                          fontFamily: F.body,
                    ...
                    <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
                      ...
                      <button
                        ...
                        onClick={async (event) => {
                          ...
                          if (!window.confirm(
                            "Remove this session plan? The Club pitch allocation will remain."
                          )) return;
          ...
          {/* Session Focus moved into Coaching overview */}
          <div className="coach-session-focus-legacy" style={{ background: P.white, borderRadius: 14, padding: 18, border: `1px solid ${P.line}`, boxShadow: Sh.card }}>
        ...
        {showLeaveForm && <div onClick={()=>setShowLeaveForm(false)} style={{position:"fixed",inset:0,zIndex:5000,background:"rgba(15,23,42,.5)",display:"grid",placeItems:"center",padding:18}}>
          <div onClick={e=>e.stopPropagation()} style={{width:"min(460px,100%)",background:P.white,borderRadius:18,padding:20,boxShadow:"0 30px 80px rgba(15,23,42,.28)"}}>
            <div style={{fontFamily:F.display,fontSize:20,fontWeight:800,color:P.ink}}>Add coach holiday</div>
...
function localDateTimeIso(dateKey, timeValue) {
  ...
  const local = new Date(year, month - 1, day, hour, minute || 0, 0, 0);
  return Number.isNaN(local.getTime()) ? null : local.toISOString();
}

function localDayIsoBounds(dateKey) {
...
function SessionBuilderScreen({ club, ageGroups, skills, allActivities, coaches, diagramMap, selectedTeam, onNav, editingSession, onClearEdit, onEditSession }) {
  ...
  const [publishedAllocation, setPublishedAllocation] = useState(null);
  ...
  const [allocationConflict, setAllocationConflict] = useState(false);
  ...
  function selectedSessionDate() {
    ...
    monday.setDate(monday.getDate() - ((monday.getDay() + 6) % 7) + (weekOffset || 0) * 7);
    monday.setDate(monday.getDate() + dayMap[day]);
  ...
  useEffect(() => {
    ...
    async function loadFacilities() {
      if (!club?.id) { setFacilities([]); return; }
      const { data } = await supabase.from("facilities").select("id,name,location,active").eq("club_id", club.id).eq("active", true).order("name");
  ...
  function removeSection(id) {
    userTouchedSessionRef.current = true;
    setHasUnsavedChanges(true); setSections((s) => s.filter((sec) => sec.id !== id)); }
  ...
  async function saveSession(forceSave = false) {
    ...
    try {
      ...
      let sessionDate = editingSession?.session_date || selectedSessionDate() || (() => { const d=new Date(); return `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,"0")}-${String(d.getDate()).padStart(2,"0")}`; })();
      let connectEventId = null;
      let connectStartsAt = editingSession?.confirmed_starts_at || editingSession?.planned_starts_at || localDateTimeIso(sessionDate, plannedStartTime);
      let connectEndsAt = null;
      let connectLocation = plannedLocation.trim() || null;
      ...
      if (editingSession) {
        ...
        const sessionUpdate = {
          total_duration_mins: totalTime,
          station_count: allDrills.length,
          notes: phasesJson,
        };

        // Club-published allocation is authoritative.
        // Only update planned time when this session has not been confirmed.
        if (!editingSession.confirmed_starts_at && editingSession.session_date && plannedStartTime) {
          sessionUpdate.planned_starts_at = localDateTimeIso(editingSession.session_date, plannedStartTime);
        }
        ...
        if (sessionId) {
          const { data: currentEvent } = await supabase.from("club_events").select("*,facility:facilities(name)").eq("session_id", sessionId).limit(1).maybeSingle();
          if (currentEvent?.id) {
            connectEventId = currentEvent.id;
            connectStartsAt = currentEvent.starts_at || connectStartsAt;
            connectEndsAt = currentEvent.ends_at || null;
            connectLocation = currentEvent.facility?.name || currentEvent.location || connectLocation;
          }
          await syncConnectSessionDraft({ club, selectedTeam, sessionId, eventId: connectEventId, sessionDate, startsAt: connectStartsAt, endsAt: connectEndsAt, location: connectLocation });
        }
      } else {
        ...
        if (!plan) {
          ...
          const { data: created, error: planErr } = await supabase.from("weekly_plans").insert({
            ...
            mode: "hurling", coach_notes: notes, published: false, starts_at: weekStart,
          }).select().single();
        ...
        planId = plan.id;
        const plannedStartIso = localDateTimeIso(sessionDate, plannedStartTime);
        const plannedFacilityId = facilities.find(
          (facility) => String(facility.name || facility.location || "") === String(plannedLocation.trim())
        )?.id || null;

        // Never create a second session for the exact same team/date/time.
        // Reuse an empty Club-created placeholder; if a real Coach session already
        // exists at that time, ask the user to edit it instead of duplicating it.
        let reusableSession = null;
        if (plannedStartIso) {
          const { data: sameDaySessions, error: sameDayError } = await supabase
            .from("sessions")
            .select("id,planned_starts_at,confirmed_starts_at,weekly_allocation_id,total_duration_mins,station_count")
            .eq("plan_id", plan.id)
            .eq("session_date", sessionDate)
            .order("created_at", { ascending: true });
          if (sameDayError) throw sameDayError;

          const plannedMs = new Date(plannedStartIso).getTime();
          const exact = (sameDaySessions || []).find((candidate) => {
            const candidateStart = candidate.confirmed_starts_at || candidate.planned_starts_at;
            if (!candidateStart) return false;
            return Math.abs(new Date(candidateStart).getTime() - plannedMs) < 60000;
          });

          if (exact) {
            const { data: existingActivities, error: existingActivitiesError } = await supabase
              .from("session_activities")
              .select("id")
              .eq("session_id", exact.id)
              .limit(1);
            if (existingActivitiesError) throw existingActivitiesError;

            if ((existingActivities || []).length > 0) {
              throw new Error("A session already exists for this team at this date and time. Open it from Sessions and choose Edit instead of creating a duplicate.");
            }
            reusableSession = exact;
          }
        }

        if (reusableSession?.id) {
          const sessionUpdate = {
            sport: selectedTeam.gender === "girls" ? "camogie" : "hurling",
            format: "stations",
            total_duration_mins: totalTime,
            station_count: allDrills.length,
            notes: phasesJson,
            planned_facility_id: plannedFacilityId,
          };
          if (!reusableSession.confirmed_starts_at) {
            sessionUpdate.planned_starts_at = plannedStartIso;
          }

          const { data: sess, error: sessionUpdateError } = await supabase
            .from("sessions")
            .update(sessionUpdate)
            .eq("id", reusableSession.id)
            .select()
            .single();
          if (sessionUpdateError || !sess?.id) {
            throw new Error("Session save failed: " + (sessionUpdateError?.message || "No session ID returned"));
          }
          sessionId = sess.id;
        } else {
          const { data: sess, error: sessionInsertError } = await supabase.from("sessions").insert({
            plan_id: plan.id,
            session_number: 1,
            sport: selectedTeam.gender === "girls" ? "camogie" : "hurling",
            format: "stations",
            ...
            planned_starts_at: plannedStartIso,
            planned_facility_id: plannedFacilityId,
          }).select().single();
          if (sessionInsertError || !sess?.id) {
            throw new Error("Session save failed: " + (sessionInsertError?.message || "No session ID returned"));
          }
          sessionId = sess.id;
        }
      }
      if (sessionId && !editingSession) {
        const { start: dayStart, end: dayEnd } = localDayIsoBounds(sessionDate);
        const { data: dayAllocations, error: allocationLookupError } = await supabase
          .from("weekly_training_allocations")
        ...
        const plannedStartForMatch = localDateTimeIso(sessionDate, plannedStartTime);
        const plannedStartMs = plannedStartForMatch ? new Date(plannedStartForMatch).getTime() : null;
        const allocation = plannedStartMs == null
          ? null
          : (dayAllocations || []).find((candidate) =>
              Math.abs(new Date(candidate.starts_at).getTime() - plannedStartMs) < 60000
            ) || null;
        ...
        if (allocation) {
          ...
          if (event?.id) {
            ...
            connectStartsAt = allocation.starts_at;
            connectEndsAt = allocation.ends_at;
            connectLocation = allocation.facility?.name || plannedLocation.trim() || null;
          ...
          const plannedStart = localDateTimeIso(sessionDate, plannedStartTime);
          const plannedEnd = plannedStart ? new Date(new Date(plannedStart).getTime() + Math.max(totalTime || 60, 1) * 60000).toISOString() : null;
          connectStartsAt = plannedStart;
          connectEndsAt = plannedEnd;
          connectLocation = plannedLocation.trim() || null;
          // A Coach session can be planned before Club confirms a slot. Only create a calendar event
          // when there is a start time; the Connect draft can still exist with Time/Location TBC.
          if (plannedStart) {
            let event = null;
            const { data: existingEvent } = await supabase.from("club_events").select("id").eq("session_id", sessionId).limit(1).maybeSingle();
            const eventPayload = {
              club_id: club.id, age_group_id: selectedTeam.id, event_type: "training", title: "Training",
            ...
            if (existingEvent?.id) {
              ...
              const { data } = await supabase.from("club_events").insert(eventPayload).select().maybeSingle();
              event = data;
            }
            if (event?.id) {
              connectEventId = event.id;
      ...
      if (sessionId && allDrills.length > 0) {
        const { error: activityInsertError } = await supabase.from("session_activities").insert(allDrills.map((d, i) => ({
          ...
          station_number: i + 1,
          sort_order: i,
          assigned_coach_id: d.coachId || null,
        })));
  ...
  useEffect(() => {
    ...
    const startsAt =
      editingSession.confirmed_starts_at ||
      editingSession.planned_starts_at ||
      null;
    ...
    const facilityId =
      editingSession.confirmed_facility_id ||
      editingSession.planned_facility_id ||
      null;
    ...
    if (facilityId) {
      ...
      if (facility) {
        setPlannedLocation(facility.name || facility.location || "");
      }
    }
  }, [
    editingSession?.id,
  ...
  return (
    <div
      onChangeCapture={() => {
        userTouchedSessionRef.current = true;
        setHasUnsavedChanges(true);
      }} style={{ flex: 1, overflow: "auto", background: P.soft }}>
      ...
      <div className="session-builder-layout" style={{ padding: "16px 20px", display: "flex", gap: 16 }}>
          ...
          <div style={{ background: P.white, borderRadius: 12, padding: 14, border: `1px solid ${P.line}`, marginBottom: 14 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)", gap: 4 }}>
              {["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"].map((d, i) => {
                ...
                return (
                  <button key={d} onClick={() => openBuilderCalendarDate(d, mon)} style={{ padding: "8px 2px", borderRadius: 8, border: isSelected ? `2px solid ${P.p600}` : isToday ? `2px solid ${P.line}` : "2px solid transparent", background: isSelected ? P.p50 : P.white, cursor: "pointer", textAlign: "center" }}>
            ...
            {publishedAllocation && (
              <div style={{ marginTop:10, padding:"9px 11px", borderRadius:9, background:allocationConflict?"#fff7ed":"#f0fdf4", border:`1px solid ${allocationConflict?"#fed7aa":"#bbf7d0"}`, fontFamily:F.body, fontSize:10, lineHeight:1.45, color:P.ink }}>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1.5fr", gap:8, marginTop:10 }}>
              ...
              <div>
                <div style={{ fontFamily:F.body, fontSize:9, fontWeight:800, color:P.muted, marginBottom:4 }}>{publishedAllocation?"CONFIRMED FACILITY":"PLANNED LOCATION (OPTIONAL)"}</div>
                <input list="spraoi-club-facilities" value={publishedAllocation ? confirmedAllocationFacility : plannedLocation}  onChange={(e)=>setPlannedLocation(e.target.value)} placeholder="Choose a Club facility or enter another location" style={{ width:"100%", boxSizing:"border-box", height:36, borderRadius:8, border:`1px solid ${P.line}`, padding:"0 9px", fontFamily:F.body, background:P.white }} />
              ...
              <div key={sec.id} style={{ marginBottom: 14 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
                  ...
                  <div style={{ marginLeft: "auto", display: "flex", gap: 2 }}>
                    ...
                    <button onClick={() => moveSection(sec.id, 1)} style={{ background: "none", border: "none", color: P.muted, cursor: "pointer", fontSize: 10 }}>▼</button>
                    <button onClick={() => removeSection(sec.id)} style={{ background: "none", border: "none", color: P.coral, cursor: "pointer", fontSize: 12 }}>x</button>
                  </div>
                </div>
                {sec.type === "stations" ? (
                  ...
                  sec.drills.length === 0 ? (
                    ...
                    sec.drills.map((d, di) => (
                      <div key={di} style={{ background: P.white, borderRadius: 10, padding: "10px 12px", border: `1px solid ${P.line}`, marginBottom: 4, display: "flex", alignItems: "center", gap: 8 }}>
                        ...
                        <button onClick={() => removeDrill(sec.id, di)} style={{ background: "none", border: "none", color: P.coral, cursor: "pointer", fontSize: 14 }}>x</button>
                      </div>
                    ))
                  )
                ) : (
        ...
        {/* Skill-led Drill Library */}
        {(() => {
          return null;
        })()}

        {/* Right — Drill Library */}
        <div className="session-drill-library" style={{ width: 260, minWidth: 0, flexShrink: 0, boxSizing: "border-box" }}>
          <div style={{ background: P.white, borderRadius: 12, padding: 14, border: `1px solid ${P.line}`, boxShadow: Sh.card, position: "sticky", top: 76 }}>
            <div style={{ fontFamily: F.display, fontSize: 14, fontWeight: 800, color: P.ink, marginBottom: 3 }}>
              Find a Drill
            </div>

            <div style={{ fontFamily: F.body, fontSize: 9, color: P.muted, marginBottom: 9 }}>
              Choose a sport, then choose the skill you want to coach.
            </div>

            <div
              className="session-sport-filters"
              style={{
                display: "grid",
              ...
              {[
                ...
                return (
                  <button
                    key={option.id}
                    onClick={() => {
                      setFilterSport(active ? "" : option.id);
                      setFilterSkill("");
                      setSearch("");
                    }}
                    style={{
                      ...
                      padding: "6px 3px",
                      borderRadius: 8,
                      border: `1.5px solid ${active ? option.color : P.line}`,
                      background: active
                        ? `${option.color}12`
                        : P.white,
                      color: active
              ...
              const availableSkills = [
                ...new Map(
                  (allActivities || [])
                    .filter((activity) => {
                      const sport =
                        activity.coach_skill?.sport || "";

                      return sport === filterSport;
                    })
                    .filter((activity) => activity.coach_skill?.id)
                    .map((activity) => [
                      activity.coach_skill.id,
                      activity.coach_skill,
                    ])
                ).values(),
              ].sort(
                (a, b) =>
              ...
              return (
                <>
                  ...
                  <input
                    ...
                    placeholder={
                      filterSkill
                        ? "Search within these drills..."
                        : "Or search skills / drills..."
                    }
                    value={search}
                    onChange={(e) =>
            ...
            <div style={{ maxHeight: 420, overflowY: "auto" }}>
              {filtered
                .filter((a) => {
                  ...
                  if (filterSkill === "games") {
                    return (
                      !a.coach_skill &&
                      String(a.sport || "").toLowerCase() ===
                        (filterSport === "athletic"
                          ? "football"
                          : filterSport)
                    );
                  }
                  ...
                  if (filterSkill) {
                    return (
                      taggedSport === filterSport &&
                      a.coach_skill?.id === filterSkill
                    );
                  }
                  ...
                  return taggedSport === filterSport;
                })
                .slice(0, 30)
                .map((a) => {
                ...
                return (
                  <div key={a.id} onClick={() => targetSection && addDrillToSection(targetSection.id, a)} style={{ padding: "7px 6px", borderBottom: `1px solid ${P.line}`, cursor: "pointer", display: "flex", alignItems: "center", gap: 8, borderRadius: 6, transition: "background .12s" }} onMouseEnter={(e) => e.currentTarget.style.background = P.soft} onMouseLeave={(e) => e.currentTarget.style.background = "transparent"}>
                    ...
                    <div style={{ minWidth: 0, flex: 1 }}>
                      ...
                      <div style={{ fontFamily: F.body, fontSize: 9, color: P.muted }}>{a.coach_skill?.name || "Game / Warm-up"} — {a.duration_mins || "?"}m</div>
                    </div>
                  </div>
                );
              })}
      ...
      {showTimeModal && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(11,37,69,.45)", zIndex: 3000, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }} onClick={() => setShowTimeModal(false)}>
          <div onClick={(e) => e.stopPropagation()} style={{ width: "min(440px, 100%)", background: P.white, borderRadius: 18, padding: 22, boxShadow: "0 24px 70px rgba(11,37,69,.25)", border: `1px solid ${P.line}` }}>
            <div style={{ width: 48, height: 48, borderRadius: 14, background: "#fff3e0", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 24, marginBottom: 14 }}>{String.fromCodePoint(0x23F1)}</div>
...
function VideoLibrary({
  ...
  function videoPickerSkills() {
    ...
    const currentSport = normalise(form.sport);

    const definitions =
      currentSport.includes("hurl") ||
      currentSport.includes("camogie")
        ? [
            ["Jab Lift", ["jab lift"]],
            ["Roll Lift", ["roll lift"]],
            ["Ground Strike", ["ground strike"]],
            ["Striking from the Hand", ["striking from the hand"]],
            ["Hand Pass", ["hand pass"]],
            ["Catching", ["chest catch", "catching"]],
            ["Overhead Catch", ["overhead catch"]],
            ["First Touch & Control", ["ball control", "controlling a moving ball"]],
            ["Solo Run", ["solo run"]],
            ["Hook", ["hook"]],
            ["Block", ["block", "frontal block"]],
            ["Goalkeeping", ["goalkeeping", "goalkeeper"]],
          ]
        : currentSport.includes("athletic")
          ? [
              ["Agility", ["agility"]],
              ...
              ["Reaction", ["reaction", "reaction speed"]],
          ...
          : [
              ["Pick Up", ["crouch lift", "pick up"]],
              ["Hand Pass", ["hand pass", "fist pass"]],
              ["Kick Pass", ["punt kick", "hook kick", "kicking"]],
              ["Catching", ["body catch", "reach catch", "low catch", "catching"]],
              ["High Catch", ["high catch"]],
              ["Solo", ["solo"]],
              ["Bounce", ["bounce"]],
              ["Shooting", ["shooting"]],
              ["Tackling", ["tackling", "checking"]],
              ["Blocking", ["block", "block down", "blocking"]],
              ["Movement & Support Play", ["feint and side step", "evasion"]],
              ["Goalkeeping", ["goalkeeping", "goalkeeper"]],
            ];
    ...
    return definitions
      .map(([cleanName, aliases]) => {
        let matched = null;

        for (const alias of aliases) {
          matched = pool.find((skill) => {
            const name = normalise(skill.name);
            const sport = normalise(skill.sport);
...
function DrillsScreen({ allActivities, diagramMap, favouriteIds, onToggleFavourite, userRole, skills = [], club }) {
  ...
  const mergedActivities = [...allActivities, ...customDrills.map((c) => ({
    id: c.id, title: c.title, description: c.description, description_html: c.descriptionHtml || null, coaching_points: c.coachingPoints, coaching_points_html: c.coachingPointsHtml || null,
  ...
  if (filterSport) {
    filtered = filtered.filter((a) => {
      ...
      if (filterSkill) {
        return (
          a.coach_skill?.sport === filterSport &&
          a.coach_skill?.id === filterSkill
        );
      }

      return a.coach_skill?.sport === filterSport;
    });
  }
        ...
        <div onClick={() => setSelectedIdx(null)} style={{ position: "fixed", inset: 0, zIndex: 99999, background: "rgba(0,0,0,0.5)", display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}>
          <div onClick={(e) => e.stopPropagation()} style={{ background: P.white, borderRadius: 16, maxWidth: 500, width: "100%", maxHeight: "85vh", overflowY: "auto", boxShadow: Sh.lift }}>
            ...
            <div style={{ padding: 18 }}>
              ...
              {selectedDrill.description && <SpraoiRichTextDisplay value={selectedDrill.description_html || selectedDrill.description} style={{ fontFamily: F.body, fontSize: 13, color: P.ink, lineHeight: 1.5, marginBottom: 14 }} />}
              {selectedDrill.coaching_points && <div style={{ marginBottom: 14 }}><div style={{ fontFamily: F.body, fontSize: 11, fontWeight: 700, color: P.p600, textTransform: "uppercase", marginBottom: 6 }}>Coaching Points</div><SpraoiRichTextDisplay value={selectedDrill.coaching_points_html || selectedDrill.coaching_points} style={{ fontFamily: F.body, fontSize: 12, color: P.ink, lineHeight: 1.6, background: P.soft, borderRadius: 10, padding: 14 }} /></div>}
...
﻿function DiagramCreator({ onSave, onClose, initialElements, backgroundImage }) {
  ...
  const restoredPitch =
    (initialElements || []).find((el) => el.type === "pitchMeta")
      ?.pitchType || "full";

  const [elements, setElements] = useState(restoredElements);
  const [pitchType, setPitchType] = useState(restoredPitch);
  ...
  const brandLogo = "/favicon-spraoi-v6.png";

  function commitElements(updater) {
    setElements((current) => {
      const next =
  ...
  function addPointElement(type, pt, extra = {}) {
    commitElements((els) => [
      ...els,
      {
        id: Date.now(),
  ...
  function handlePointerUp(e) {
    if (dragging) {
      ...
      if (
        ...
        setHistory((items) =>
          [...items, dragSnapshot.current].slice(
            -40
          )
        );
        setFuture([]);
  ...
  function renderPitch() {
    ...
    const halfPitch =
      pitchType === "half" ||
      pitchType === "attackHalf";

    const thirdPitch =
      pitchType === "third";

    const startX = halfPitch
      ? 14
      : thirdPitch
        ? 14
        : 14;

    const endX = halfPitch
      ? 426
      : thirdPitch
        ? 300
        : 426;
      ...
      >
        ...
        >
          ...
          >
              ...
              if (el.type === "cone") {
                return (
                  <g key={el.id}>
                    ...
                    <polygon
                      points={`${el.x},${el.y - 10} ${el.x - 7},${el.y + 5} ${el.x + 7},${el.y + 5}`}
                      fill="#FF7A00"
                      stroke="#fff"
                      strokeWidth="1.5"
                    />
              ...
              if (el.type === "sliotar") {
                return (
                  <g key={el.id}>
                    ...
                    />
                    <path
                      d={`M${el.x - 4},${el.y} Q${el.x},${el.y - 3} ${el.x + 4},${el.y}`}
                      fill="none"
                      stroke="#d32f2f"
                      strokeWidth="1"
                    />
              ...
              if (el.type === "arrow") {
                ...
                return (
                  <g key={el.id}>
                    ...
                    <polygon
                      points={`${el.x2},${el.y2} ${ax + px},${ay + py} ${ax - px},${ay - py}`}
            ...
            {/* Remaining equipment selection overlay V3 */}
            {(() => {
              const selected = elements.find(
                (item) => item.id === selectedEl
              );
            ...
            {/* End remaining equipment selection overlay V3 */}
          ...
          >
            {backgroundImage &&
              elements.length === 0 && (
                ...
                >
                  Previous diagram shown
                  faintly as a reference.
                </div>
              )}

            {arrowStart
              ? "Click to set the arrow end point"
              : tool === "select"
                ? "Select and drag objects. Ctrl+D duplicates. Delete removes. Ctrl+Z undoes."
                : `Click on the pitch to place ${tool}`}
          </div>
        </div>
      </div>
    </div>
...
function spraoiRichHtml(value) {
  const raw = String(value || "");
  if (!raw) return "";

  const looksLikeHtml = /<(p|div|br|strong|b|em|i|u|ul|ol|li)\b/i.test(raw);
...
function spraoiRichPlain(value) {
  const html = spraoiRichHtml(value);
...
function SpraoiRichTextDisplay({ value, style }) {
  if (!value) return null;
  return <div style={style} dangerouslySetInnerHTML={{ __html: spraoiRichHtml(value) }} />;
}

function SpraoiRichTextEditor({
  value,
  onChange,
  placeholder,
}) {
...
const DESCRIPTIVE_DRILL_TITLE_ALIASES = {
  "Underarm Throw - Knock Down Target Circle": "Alien Attack",
  "Hurley Balance - Hidden Beanbag Cone Relay": "BeanBag Bonanza",
  "Hurley Balance - Moving Beanbag Control": "Beanbag Boogie",
  "Hurley Balance - Opposed Beanbag Control": "Beanbag Joust",
  "Hurley Balance - Colour Cone Criss-Cross Relay": "Colour Criss Cross",
  "Cone Lift - Scoop & Return Relay": "Cone Scoop",
  "Ground Strike - Continuous Two-Team Clearance": "Don't let the ball stop",
  "Hurley Balance - Beanbag Control Relay": "Flip the pancake",
  "Ground Strike - 2v2 Goal Scoring Game": "Goals Galore",
  "Dribble - Multi-Gate Dribble Challenge": "Goals Goals Goals",
  "Cone Lift - Cone Pick-Up Relay": "Hook a Duck",
  "Jab Lift - Lift & Return Relay": "Jab it",
  "Dribble - Ball Protection Ring Game": "King or Queen of the Ring",
  "Jab Lift - Cone & Hoop Lift Challenge": "Lifting Eggs",
  "Dribble - Control & Chase Grid": "Pac Man",
  "Jab Lift - Partner Scoop Challenge": "Partner Scoop",
  "Ground Strike - Gate Passing Rally": "Rally Alley",
  "Roll Lift - Roll Back & Cone Lift": "Roll Back and Flip Over",
  "Strike from the Hand - Wall Strike Technique": "Salute the Strike",
  "Target Accuracy - Strike the Targets": "Skittles",
  "Throwing Accuracy - Two-Team Clearance Game": "Smelly Socks",
  "Team Coordination - Multi-Stage Ball & Hurley Relay": "The Wheel",
  "Spatial Awareness - Colour Cone Movement Tasks": "Traffic Jammer",
  "Target Accuracy - Chute-to-Cone Target Game": "Aim and Shoot",
  "Ball Control - Around-the-Body Movement": "Around the World",
  "Passing - Circle Pass vs Runner Race": "Beat the Circle",
  "Target Accuracy - Move the Large Ball": "Big Ball Bombardment",
  "Target Accuracy - Two-Team Large Ball Push": "Boulder Bowling",
  "Ball Control - Bubble Tracking & Striking": "Bubble World",
  "Passing - Captain Target Passing Game": "Captain Ball",
  "Movement - Partner Follow & Mirror": "Chasing Shadows",
  "Passing - Clear & Return Ball Game": "Clear the Circle",
  "Agility - Traffic-Light Movement Course": "Crazy Cars",
  "Passing - Moving Gate Passing Challenge": "Find the Goal",
  "Coordination - Hoop Stepping Race": "Frog Race",
  "Target Accuracy - Moving Hoop Passing": "Hoop Hurrah",
  "Team Coordination - Linked Circle Hoop Pass": "Hula Hoop Loop",
  "Throwing Accuracy - Javelin Through Hoops": "Javelin",
  "Ball Control - Team Keep-Up Circle": "Keepie Uppies",
  "Agility - Colour Cone Reaction Course": "Minefield",
  "Coordination - Partner Mirror Movement": "Mirror Mirror",
  "Team Coordination - Parachute Movement & Ball Control": "Parachute Play",
  "Throwing Accuracy - Progressive Hoop Targets": "Pot of Gold",
  "Balance - Stepping-Stone Ball Carry": "Rescue Ball",
  "Passing - Line Passing Relay": "Speed Lines",
  "Speed - Hoop-to-Hoop Ball Relay": "Swap Shop",
  "Support Play - Wide-Zone Conditioned Game": "Wide Player",
  "Reaction Speed - Colour Cone Collection Race": "Colour Grab",
  "Chest Catch - Circle Catch vs Runner Race": "Chest Catch - Beat the Circle",
  "Chest Catch - Captain Target Catch Game": "Chest Catch - Captain Ball",
  "Hook - Timed Chase & Hook Race": "Hook - A Race against Time",
  "Hook - Chase & Hook Tag": "Hook - Catch Me if You Can",
  "Dribble - Fill-the-Circle Possession Game": "Dribble - Fill the Circle",
  "Stopping a Ground Ball - Goal-to-Goal Blocking": "Stopping a Ground Ball - Goal to Goal",
  "Stopping a Ground Ball - Guarded Lane Blocking": "Stopping a Ground Ball - Pass the Guards",
  "Stopping a Ground Ball - Central Defender Blocking": "Stopping a Ground Ball - Piggy in the Middle",
  "Ground Strike - Accuracy Through Gates": "Strike on the Ground - Golf Goals",
  "Ground Strike - Four-Goal Scoring Game": "Strike on the Ground - Four Goal Game",
  "Frontal Ground Block - Points-Based Blocking Game": "Frontal Ground Block - Skill Point Game",
  "Strike from the Hand - Wide Target Striking": "Strike from the Hand - Keep it Wide",
  "Strike from the Hand - Possession Striking Game": "Strike from the Hand - Keep Ball",
  "Strike from the Hand - Points-Based Striking Game": "Strike from the Hand - Skill Point Game",
  "Ground Strike on the Run - Wide Target Striking": "Ground Strike On the Run - Keep it Wide",
  "Ground Strike on the Run - Rounders Game": "Ground Strike On the Run - Ground Strike Rounders",
  "Roll Lift - Moving Cone Reaction Game": "Roll Lift - Musical Chairs",
  "Overhead Catch - Points-Based Catching Game": "Overhead Catch - Skill Point Game",
  "Jab Lift - Moving Cone Reaction Game": "Jab Lift - Musical Chairs",
  "Solo Run - Central Channel Solo": "Solo Run - Through the Middle",
  "Solo Run - Grid-to-Grid Solo": "Solo Run - Grid Swap",
  "Solo Run - Tag & Evasion Game": "Solo Run - Solo Tag",
  "Ground Flick - Possession Game": "Ground Flick - Keep Ball",
  "Ground Flick - 1v1 Opposed Flick": "Ground Flick - One on One",
  "Ground Flick - Points-Based Flicking Game": "Ground Flick - Skill Point Game",
...
function getActivityDiagram(activity, diagramMap) {
  ...
  return (
    diagramMap[activity.title] ||
    diagramMap[DESCRIPTIVE_DRILL_TITLE_ALIASES[activity.title]] ||
    null
  );
}
...
function DrillCardBuilder({ diagramMap, allActivities, userRole, copyFrom, onBack }) {
  ...
  const [selectedDiagram, setSelectedDiagram] = useState(() => {
    if (copyFrom && getActivityDiagram(copyFrom, diagramMap)) return getActivityDiagram(copyFrom, diagramMap);
  ...
  function saveCard() {
    ...
    if (!spraoiRichPlain(description).trim()) { showToast("Add a description"); return; }
    if (!spraoiRichPlain(coachingPoints).trim()) { showToast("Add at least one coaching point"); return; }
    ...
    const card = { id: editingIdx !== null ? savedCards[editingIdx].id : Date.now().toString(), title: title.trim(), description: spraoiRichPlain(description), descriptionHtml: spraoiRichHtml(description), coachingPoints: spraoiRichPlain(coachingPoints), coachingPointsHtml: spraoiRichHtml(coachingPoints), setup, equipment, sport, category, difficulty, duration: parseInt(duration) || 10, diagram: selectedDiagram, customDiagram: customDiagram || null, diagramElements: diagramElements.length > 0 ? diagramElements : null, createdAt: editingIdx !== null ? savedCards[editingIdx].createdAt : new Date().toISOString() };
    let updated;
  ...
  function editCard(idx) {
    ...
    if (!proceed) return;
    setTitle(c.title || ""); setDescription(c.descriptionHtml || c.description || ""); setCoachingPoints(c.coachingPointsHtml || c.coachingPoints || "");
  ...
  function copyFromExisting(activity) {
    setTitle((activity.title || "") + " (Copy)"); setDescription(activity.description_html || activity.description || "");
    setCoachingPoints(activity.coaching_points_html || activity.coaching_points || ""); setSetup(activity.setup || "");
    ...
    if (getActivityDiagram(activity, diagramMap)) setSelectedDiagram(getActivityDiagram(activity, diagramMap));
  }
  ...
  return (
    <div style={{ padding: "16px 24px" }}>
      ...
      <div style={{ marginBottom: 14 }}>
        <button onClick={onBack} style={{ padding: "6px 14px", borderRadius: 8, border: `1.5px solid ${P.line}`, background: P.white, fontFamily: F.body, fontSize: 11, fontWeight: 700, color: P.muted, cursor: "pointer" }}>{String.fromCodePoint(0x2190)} Back</button>
      </div>
      <div style={{ display: "flex", gap: 20 }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ background: P.white, borderRadius: 14, padding: 18, border: `1px solid ${P.line}`, boxShadow: Sh.card }}>
            ...
            <SpraoiRichTextEditor value={description} onChange={setDescription} placeholder="Describe the drill..." />
            ...
            <SpraoiRichTextEditor value={coachingPoints} onChange={setCoachingPoints} placeholder="Add clear coaching points..." />
            ...
            placeholder="Describe how to set up the drill..."
            rows={3}
            style={{
              display: "block",
              width: "100%",
          ...
          />
            ...
            placeholder="e.g. cones, footballs, bibs"
            style={{
              display: "block",
              width: "100%",
              maxWidth: "100%",
          ...
          <div style={{ position: "sticky", top: 76 }}>
            ...
            <div ref={cardRef} style={{ background: P.white, borderRadius: 14, border: `1.5px solid ${P.line}`, overflow: "hidden", boxShadow: Sh.card }}>
              ...
              <div style={{ padding: "12px 14px" }}>
                ...
                {description && <SpraoiRichTextDisplay value={description} style={{ fontFamily: F.body, fontSize: 11, color: P.ink, lineHeight: 1.5, marginBottom: 8 }} />}
                {coachingPoints && (
                  <div style={{ marginBottom: 8 }}>
                    ...
                    <div style={{ fontFamily: F.body, fontSize: 10, color: P.ink, lineHeight: 1.5, background: P.soft, borderRadius: 6, padding: 8 }}>
                      <SpraoiRichTextDisplay value={coachingPoints} />
                    </div>
                  </div>
                )}
                {setup && (
                  ...
                  {savedCards.map((c, idx) => {
                    ...
                    return (
                      <div key={c.id || idx} style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 10px", background: P.white, borderRadius: 8, border: `1px solid ${P.line}`, marginBottom: 6, cursor: "pointer" }} onClick={() => editCard(idx)}>
                        ...
                        <button onClick={(e) => { e.stopPropagation(); deleteCard(idx); }} style={{ background: "none", border: "none", color: P.coral, cursor: isSuperAdmin ? "pointer" : "not-allowed", fontSize: 14, padding: "2px 4px", opacity: isSuperAdmin ? 1 : 0.3 }} title={isSuperAdmin ? "Delete" : "Admin only"}>x</button>
                      </div>
                    );
                  })}
                </div>
...
async function loadTeamSessions(ageGroupId) {
...
function SessionsListScreen({
  ...
  useEffect(() => {
    ...
    async function loadSessions() {
      ...
      try {
        const rows = await loadTeamSessions(selectedTeam.id);
        if (cancelled) return;
        setSessions(rows);
      } catch (error) {
        if (!cancelled) {
  ...
  async function removeSession(session) {
    ...
    try {
      ...
      if (sessionError) throw sessionError;
      if (!deleted?.length) throw new Error("The session could not be removed. Check the session delete policy.");
      setSessions((rows) => rows.filter((row) => row.id !== session.id));
    } catch (error) {
      alert("Could not remove session: " + (error?.message || "Unknown error"));
    }
  }
  ...
  function SessionRow({ session }) {
    ...
    const time = start ? (() => {
      ...
      return Number.isNaN(value.getTime()) ? null : value.toLocaleTimeString("en-IE", { hour: "2-digit", minute: "2-digit" });
    })() : null;
      ...
      >
        ...
        >
          ...
          <button
            ...
            style={{ height: 34, padding: "0 10px", borderRadius: 8, border: `1px solid ${P.line}`, background: P.white, color: P.coral, fontFamily: F.body, fontSize: 10, fontWeight: 700, cursor: "pointer" }}
          >
            Remove
          </button>
        </div>
...
function getAcademyWeeklyRecommendations(planSessions, skills = [], overrides = {}, selectedTeam = null) {
  ...
  return ["football", "hurling"].map((code) => {
    ...
    /*
     * Academy recommendation priority:
     * 1. Explicit coach override is applied below.
     * 2. Stable linked skill IDs from Coach activities.
     * 3. Existing ranked/text matching.
     * 4. Existing combined-text fallback.
     *
     * This keeps Academy recommendations stable when drill titles
     * or display labels change.
     */
    const linkedSkillCounts = new Map();

    sourceActivities.forEach((activity) => {
      const linkedSkillId =
        activity?.skillId ||
        activity?.skill_id ||
        activity?.skillObject?.id ||
        activity?.skill?.id ||
        null;

      if (!linkedSkillId) return;

      linkedSkillCounts.set(
        linkedSkillId,
        (linkedSkillCounts.get(linkedSkillId) || 0) + 1
      );
    });

    const rankedLinkedSkills = [...linkedSkillCounts.entries()]
      .map(([skillId, frequency]) => {
        const skill = codeSkills.find(
        ...
        return skill
          ? {
              skill,
              frequency,
              totalScore: 1000 + frequency,
              sources: sourceActivities.filter((activity) => {
                const activitySkillId =
                  activity?.skillId ||
                  activity?.skill_id ||
                  activity?.skillObject?.id ||
                  activity?.skill?.id ||
                ...
                return activitySkillId === skillId;
              }),
            }
          : null;
      })
  ...
  return (
    <div style={{ flex: 1, minWidth: 0, overflow: "auto", background: "#f7f9fc" }}>
      <TopBar title="Academy Admin" sub={`${teamName} — ${weekLabel}`}>
        ...
        <Btn label="Manage Weekly Content" variant="primary" onClick={() => onNav("academy-content")} />
      </TopBar>
            ...
            <section style={{ background: P.white, borderRadius: 16, border: `1px solid ${P.line}`, boxShadow: Sh.card, overflow: "hidden" }}>
              ...
              {weeklyRecommendations.length > 0 ? (
                ...
                <div style={{ padding: "28px 18px", textAlign: "center" }}>
                  <div style={{ width: 46, height: 46, borderRadius: 13, background: academySoft, color: academyBlue, display: "grid", placeItems: "center", fontSize: 20, margin: "0 auto 11px" }}>ï¼‹</div>
...
const ACADEMY_TEMPLATES = [
  ...
  { type: "exercise", icon: "⭐", title: "Star jumps", instruction: "Complete 3 sets with a short rest.", xp: 10, target: "3 x 20" },
  { type: "exercise", icon: "🦵", title: "Lunges", instruction: "Keep your chest tall and alternate legs.", xp: 10, target: "20 each side" },
  { type: "exercise", icon: "⬇", title: "Squats", instruction: "Sit back, keep your knees tracking forward.", xp: 10, target: "3 x 15" },
  ...
  return (
    <div style={{ flex: 1, overflow: "auto", background: P.soft }}>
      ...
      <div style={{ padding: 24, maxWidth: 1180, margin: "0 auto", display: "grid", gap: 16 }}>
        <AcademyCard>
          <div style={{ fontFamily: F.display, fontSize: 18, fontWeight: 800, color: P.ink }}>Add a player</div>
          ...
          <div style={{ display: "flex", gap: 8, marginTop: 14, flexWrap: "wrap" }}>
            ...
            <button onClick={addPlayer} disabled={saving || !name.trim() || !selectedTeam?.id || !club?.id} style={{ height: 38, border: 0, borderRadius: 10, background: ACADEMY_BLUE, color: "#fff", padding: "0 16px", fontFamily: F.body, fontSize: 11, fontWeight: 800, cursor: "pointer", opacity: saving || !name.trim() || !selectedTeam?.id || !club?.id ? .55 : 1 }}>{saving ? "Adding…" : "ï¼‹ Add player"}</button>
          </div>
        ...
        <AcademyCard>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10, marginBottom: 12 }}>
            <div>
              <div style={{ fontFamily: F.display, fontSize: 17, fontWeight: 800, color: P.ink }}>Team players</div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(240px,1fr))", gap: 10 }}>
              {players.map((player) => {
                ...
                return <div key={player.id} style={{ border: `1px solid ${P.line}`, borderRadius: 13, padding: 13, display: "flex", alignItems: "center", gap: 11 }}>
                  <div style={{ width: 40, height: 40, borderRadius: "50%", background: ACADEMY_SOFT, color: ACADEMY_BLUE, display: "grid", placeItems: "center", fontWeight: 800 }}>{player.name?.[0] || "P"}</div>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontFamily: F.body, fontSize: 12, fontWeight: 800, color: P.ink }}>{player.name}</div>
                    <div style={{ fontFamily: F.body, fontSize: 10, color: P.muted, marginTop: 2 }}>{linked ? "Parent linked" : "Awaiting parent"}</div>
                  </div>
                  <button onClick={() => removePlayer(player.id)} title="Remove player" style={{ border: 0, background: "transparent", color: "#b91c1c", cursor: "pointer", fontSize: 16 }}>x</button>
                </div>;
              })}
            </div>
          )}
...
function AcademyParents({ selectedTeam, parentRows, setParentRows }) {
  ...
  const academyChildBase = import.meta.env.VITE_ACADEMY_CHILD_URL || "https://academy.spraoisports.com";
  ...
  return (
    <div style={{ flex: 1, overflow: "auto", background: P.soft }}>
      ...
      <div style={{ padding: 24, maxWidth: 1180, margin: "0 auto", display: "grid", gap: 16 }}>
        ...
        <AcademyCard>
          <div style={{ fontFamily: F.display, fontSize: 18, fontWeight: 800, color: P.ink }}>Team parent link</div>
          ...
          <div style={{ display: "flex", gap: 8, marginTop: 12, alignItems: "center", flexWrap: "wrap" }}>
            ...
            <button onClick={() => copy(teamLink)} style={{ height: 36, border: 0, borderRadius: 9, background: ACADEMY_BLUE, color: "#fff", padding: "0 13px", fontFamily: F.body, fontSize: 10, fontWeight: 800, cursor: "pointer" }}>Copy link</button>
          </div>
        </AcademyCard>
        <AcademyCard>
          <div style={{ display: "grid", gridTemplateColumns: "1fr auto", gap: 16, alignItems: "center" }}>
            <div>
              <div style={{ fontFamily: F.display, fontSize: 18, fontWeight: 800, color: P.ink }}>Parent access is managed in Spraoi Club</div>
            ...
            <label style={{ height: 38, padding: "0 14px", borderRadius: 10, background: ACADEMY_BLUE, color: "#fff", display: "flex", alignItems: "center", fontFamily: F.body, fontSize: 11, fontWeight: 800, cursor: "pointer" }}>
...
export function CoachModule({
  ...
  return (
    <>
      ...
      {screen === "coach-planner" && <PlannerScreen onNav={onNav} club={club} ageGroups={ageGroups} upcomingSessions={upcomingSessions} onOpenSession={openSession} allActivities={allActivities} coaches={selectedTeamCoaches} skills={skills} diagramMap={diagramMap} selectedTeam={selectedTeam} />}
      {screen === "coach-sessions" && <SessionsListScreen
        club={club}
        selectedTeam={selectedTeam}
...
export default function App() {
  ...
  async function loadUserRole(userId, userEmail) {
    ...
    try {
      ...
      // Merge legacy coach_assignments as well. During migration some users have
      // valid teams in both tables; treating one table as an either/or source made
      // the Coach selector lose teams and display a different team from Dashboard.
  ...
  // Keep exactly one active Coach team. The selector, Dashboard, Planner,
  // Sessions and Builder all read this same selectedTeam state.
  useEffect(() => {
    selectedTeamIdRef.current = selectedTeam?.id || null;
  }, [selectedTeam?.id]);

  useEffect(() => {
    if (authLoading || !ageGroups.length) return;

    const allowedTeams = myTeams?.length
    ...
    const currentAllowed = allowedTeams.find(
  ...
  }, [authLoading, ageGroups, myTeams, selectedTeam?.id, club?.id]);
  ...
  useEffect(() => {
    const syncFromStorage = (event) => {
      if (
        event.type === "storage" &&
        event.key &&
        ![ACTIVE_TEAM_KEY, "spraoi_team_id", ACTIVE_CLUB_KEY].includes(event.key)
      ) return;
      ...
      if (!teamId || !ageGroups.length || !myTeams?.length) return;

      const found = ageGroups.find(
        (ag) =>
          String(ag.id) === String(teamId) &&
          myTeams.some((id) => String(id) === String(ag.id))
      );

      if (found && String(found.id) !== String(selectedTeamIdRef.current || "")) {
        selectedTeamIdRef.current = found.id;
  ...
  useEffect(() => {
    ...
    // Clear the previous team's derived UI immediately so it can never appear
    // underneath the new selector while the new requests are loading.
    setUpcomingSessions([]);
    setWeeklyPlan(null);
    setPlanSessions([]);
    ...
    if (!teamId) return;
    loadUpcoming(teamId);
    loadAcademyCoachPlan(teamId);
  }, [selectedTeam?.id]);

  function selectTeam(ag) {
    if (!ag?.id) return;
    selectedTeamIdRef.current = ag.id;
    setSelectedTeam(ag);
  ...
  async function loadActivities() {
    const [{ data: activitiesData }, { data: tagData }] = await Promise.all([
      supabase
        .from("activities")
        .select("*, skill:skills!activities_skill_id_fkey(id, name, sport, category, video_url)")
        .order("title"),

      supabase
        .from("activity_skill_tags")
        .select("activity_id, is_primary, confidence, coach_skill:coach_skill_taxonomy(id, name, sport, category, sort_order)")
        .eq("is_primary", true)
    ]);

    const primaryTagByActivity = new Map(
      (tagData || [])
        .filter((row) => row.activity_id && row.coach_skill)
        .map((row) => [row.activity_id, row])
    );

    const enriched = (activitiesData || []).map((activity) => {
      const tag = primaryTagByActivity.get(activity.id);

      return {
        ...activity,
        coach_skill: tag?.coach_skill || null,
        coach_skill_confidence: tag?.confidence || 0,
      };
    });
  ...
  async function loadUpcoming(ageGroupId) {
    if (!ageGroupId) return;
    const requestTeamId = String(ageGroupId);
    try {
      const rows = await loadTeamSessions(ageGroupId);
      if (String(selectedTeamIdRef.current || "") !== requestTeamId) return;
      setUpcomingSessions(rows);
    } catch (error) {
      console.error("Coach session load failed:", error);
      if (String(selectedTeamIdRef.current || "") === requestTeamId) {
        setUpcomingSessions([]);
      }
    }
  }
  ...
  async function loadAcademyCoachPlan(ageGroupId) {
    if (!ageGroupId) return;
    const requestTeamId = String(ageGroupId);
    const isCurrent = () => String(selectedTeamIdRef.current || "") === requestTeamId;
    ...
    try {
      ...
      const { data: savedExtras } = await supabase
      ...
      if (!isCurrent()) return;
      if (savedExtras) {
        setAcademyExtras(savedExtras.map(x => ({
          ...x,
          xp: x.xp_reward,
          type: x.activity_type || "exercise",
      ...
      if (activityIds.length) {
        const { data } = await supabase
          .from("activities")
          .select("*, skill:skills(id, name, sport, category, video_url)")
          .in("id", activityIds);
        activities = data || [];
      }
      if (!isCurrent()) return;
      ...
      const hydrated = sessionList.map(sess => ({
        ...sess,
        session_activities: (links || [])
          .filter(x => x.session_id === sess.id)
          .map(x => ({
            ...x,
            activity: amap[x.activity_id] || { id: x.activity_id, title: "Coach drill" },
          })),
      }));
      ...
      <div style={{ minHeight: "100vh", background: P.soft, fontFamily: F.body }}>
        <div style={{ maxWidth: 580, margin: "0 auto", padding: "20px 16px" }}>
          ...
          {activities.map((sa, i) => {
            ...
            const df = getActivityDiagram(act, diagramMap);
            return (
      ...
      <div style={{ minHeight: "100vh", background: P.navy, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: F.body, padding: 16 }}>
        <div style={{ maxWidth: 380, width: "100%" }}>
          <div style={{ textAlign: "center", marginBottom: 28 }}>
            ...
            <div style={{ fontFamily: F.body, fontSize: 12, color: "rgba(255,255,255,.5)", marginTop: 4 }}>Club staff sign in</div>
          </div>
          <div style={{ background: P.white, borderRadius: 18, padding: 28, boxShadow: Sh.lift }}>
            ...
            <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="Email address" style={{ width: "100%", boxSizing: "border-box", padding: "12px 14px", borderRadius: 10, border: `1.5px solid ${P.line}`, fontFamily: F.body, fontSize: 13, marginTop: 4, marginBottom: 12, background: P.soft }} />
  ...
  if (!selectedTeam && ageGroups.length > 0) {
    ...
    // Has teams — the selected-team effect above restores the saved/first
    // allowed team. Never call setState/selectTeam during render.
    return null;
  }
  ...
  return (
    <div className="spraoi-shell" style={{ display: "flex", minHeight: "100vh", width: "100%", fontFamily: F.body, paddingTop: showMobile ? 116 : 0, paddingBottom: showMobile ? 78 : 0, boxSizing: "border-box" }}>
      ...
      {screen === "coach-planner" && <PlannerScreen onNav={setScreen} club={club} ageGroups={ageGroups} upcomingSessions={upcomingSessions} onOpenSession={openSession} allActivities={allActivities} coaches={selectedTeamCoaches} skills={skills} diagramMap={diagramMap} selectedTeam={selectedTeam} />}
      {screen === "coach-sessions" && <SessionsListScreen
        club={club}
        selectedTeam={selectedTeam}
        upcomingSessions={upcomingSessions}
