...
const C = {
  turf: EVENT_CONFIG.colors.dark,
  pitch: EVENT_CONFIG.colors.primary,
  pitchLight: EVENT_CONFIG.colors.primaryDark,
  line: EVENT_CONFIG.colors.background,
  ash: "#AE7D64",
  sliotar: EVENT_CONFIG.colors.accent,
  ink: EVENT_CONFIG.colors.text,
  inkSoft: EVENT_CONFIG.colors.textMuted,
};
...
`;
const HERO_BRIGHT = EVENT_CONFIG.heroBright;
const HERO_DARK = EVENT_CONFIG.heroDark;

const CREST_VERSION = "6";
const BADGE_LOGO = `/logo.png?v=${CREST_VERSION}`;

const CRESTS = Object.fromEntries(
  Object.entries(EVENT_CONFIG.crests).map(([id, path]) => [id, path ? `${path}?v=${CREST_VERSION}` : ""])
);

/* ---------- Event constants ---------- */
const EVENT = {
  name: `${EVENT_CONFIG.hostClub} ${EVENT_CONFIG.name}`,
  date: EVENT_CONFIG.date,
  venue: EVENT_CONFIG.venue,
  registration: EVENT_CONFIG.registration,
  procession: EVENT_CONFIG.procession,
  firstThrowIn: EVENT_CONFIG.firstThrowIn,
  targetFinish: EVENT_CONFIG.targetFinish,
};
const EVENT_YEAR = EVENT_CONFIG.eventYear;
const EVENT_MONTH = EVENT_CONFIG.eventMonth;
const EVENT_DAY = EVENT_CONFIG.eventDay;
function isEventDay(d = new Date()) {
  return d.getFullYear() === EVENT_YEAR && d.getMonth() === EVENT_MONTH && d.getDate() === EVENT_DAY;
}

const WELCOME_PARAGRAPHS = EVENT_CONFIG.welcomeParagraphs;
const WELCOME_SIGNOFF = EVENT_CONFIG.welcomeSignoff;

const DEFAULT_CLUBS = EVENT_CONFIG.clubs;
...
const DEFAULT_SPONSORS = EVENT_CONFIG.defaultSponsors;
...
const ADMIN_ACCOUNTS = EVENT_CONFIG.admins;
...
const REFEREE_SECRET = EVENT_CONFIG.refereeSecret;
...
const CLUB_PASSWORDS = EVENT_CONFIG.clubPasswords;
...
const SAUSAGE_BAP_PRICE = EVENT_CONFIG.sausageBapPrice;

// TENTATIVE — confirm the real cutoff date. Orders lock at the end of this day.
const ORDER_LOCK_DATE = new Date(EVENT_CONFIG.orderLockDate);
...
function InfoScreen({ sponsors, announcements, myClubObj, onMentorClick }) {
  const items = [
    ...
    },
    {
      ...
      body: "Limited car parking is available at ${EVENT_CONFIG.hostClub}, and buses are welcome to park on site. Overflow parking has been kindly provided by the HSE at Swords Business Campus, a short ten-minute walk from the grounds - stewards will be on duty at both locations to guide you.",
      maps: [
...
const PITCHES = EVENT_CONFIG.pitches;
const SLOT_MINUTES = EVENT_CONFIG.slotMinutes;
const START_HOUR = EVENT_CONFIG.startHour;
const START_MIN = EVENT_CONFIG.startMin;
...
const LUNCH_MINUTES = EVENT_CONFIG.lunchMinutes;
const MATCH_DURATION_MIN = EVENT_CONFIG.matchDurationMin;
const PRESENTATION_MINUTES = EVENT_CONFIG.presentationMinutes;
